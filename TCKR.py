...
def fetch_all_stock_prices(tickers, api_key):
    prices = {}
    batch_size = 10
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for i in range(0, len(tickers), batch_size):
            batch = tickers[i:i+batch_size]
            futures = [executor.submit(fetch_finnhub_quote, ticker, api_key) for ticker in batch]
...
def fetch_all_stock_prices_with_429(tickers, api_key):
    prices = {}
    batch_size = 10
    had_429 = False
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for i in range(0, len(tickers), batch_size):
            batch = tickers[i:i+batch_size]
            futures = [executor.submit(fetch_with_status, ticker, api_key) for ticker in batch]
            for future in concurrent.futures.as_completed(futures):
...
class PriceFetchWorker(QtCore.QThread):
    ...
    def run(self):
        prices = fetch_all_stock_prices(self.tickers, self.api_key)
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_settings(self):
        dlg = SettingsDialog(self.ticker_window)
        if dlg.exec_():
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def setup_appbar_and_position(self):
        ...
        QtCore.QTimer.singleShot(200, after_removal)
    ...
    def update_prices_full(self):
        ...
        if not api_key:
            ...
            self.gl_widget.update()
            return
        self.worker = PriceFetchWorker(sorted([s[0] for s in load_stocks()]), api_key)
    ...
    def update_prices_inplace(self):
        now = time.time()
        print(f"[BACKOFF DEBUG] update_prices_inplace called at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(now))}")
        print(f"[BACKOFF DEBUG] Current backoff_until: {getattr(TickerWindow, 'backoff_until', 0)}")
        if hasattr(TickerWindow, 'backoff_until') and now < TickerWindow.backoff_until:
            print(f"[BACKOFF DEBUG] Skipping fetch, backoff active until {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(TickerWindow.backoff_until))}")
            return

        api_key = ensure_finnhub_api_key(self)
        if not api_key:
            print("[BACKOFF DEBUG] No API key, aborting fetch.")
            return

        # Run fetch in a worker thread to avoid blocking the UI
        def fetch_and_handle():
            tickers = self.stocks
            prices, had_429 = fetch_all_stock_prices_with_429(tickers, api_key)
    ...
    def _handle_prices_inplace(self, prices, had_429, now):
        if had_429:
            ...
            print(f"[BACKOFF DEBUG] 429 error detected. Consecutive cycles: {TickerWindow.consecutive_429_cycles}")
            if TickerWindow.consecutive_429_cycles >= 2:
                TickerWindow.backoff_until = now + 300  # 5 minutes
                print(f"[BACKOFF DEBUG] Two consecutive cycles with 429 errors. Backing off for 5 minutes until {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(TickerWindow.backoff_until))}")
            ...
            if getattr(TickerWindow, 'consecutive_429_cycles', 0) > 0 or getattr(TickerWindow, 'backoff_until', 0) > 0:
                print("[BACKOFF DEBUG] No 429 error, resetting backoff process.")
            TickerWindow.consecutive_429_cycles = 0
    ...
    def on_prices_inplace_fetched(self, new_prices):
        ...
        for tkr, (price, prev_close) in new_prices.items():
            ...
            print(f"[DEBUG] {tkr}: old_price={old_price}, new_price={price}")
            if price is not None and (old_price is None or price != old_price):
                print(f"[DEBUG] {tkr} price changed!")
    ...
    def update_prices(self):
        ...
        if not api_key:
            return
        self.stocks = sorted([s[0] for s in load_stocks()])
        self.prices = fetch_all_stock_prices(self.stocks, api_key)
        self.build_ticker_text()
    ...
    def paint_ticker(self, widget):
        ...
        led_color = QtGui.QColor(30, 40, 60)
        off_color = QtGui.QColor(10, 12, 18)
        grad = QtGui.QLinearGradient(0, 0, 0, height)
        grad.setColorAt(0, led_color)
        grad.setColorAt(1, off_color)
