...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0123.2100")
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, is_secondary=False):
        ...
        # Per-instance cache for rendered text pixmaps (text+glow)
        # Keyed by (text, font.toString(), text_color_rgba, glow_color_rgba_or_None)
        self._text_pixmap_cache = {}
    ...
    def draw_text_with_global_glow(self, painter, x, y, text, text_color, glow_color=None, settings=None):
        ...
        # Attempt to use cached pixmap for text+glow to avoid repeated drawText calls
        try:
            font = painter.font()
            key = (text, font.toString(), int(text_color.rgba()), int(glow_color.rgba()) if glow_color else None)
            cached = self._text_pixmap_cache.get(key)
            if cached is None:
                # create pixmap
                ...
                # padding for glow halo
                pad = 4
                pm_w = text_width + pad * 2
                pm_h = text_height + pad * 2
                ...
                if glow_color:
                    ...
                    if USE_OPT:
                        for dx, dy in opt.get_subtle_glow_offsets():
                            p.drawText(pad + dx, pad + ascent + dy, text)
                    else:
                        for dx in [-1, 0, 1]:
                            for dy in [-1, 0, 1]:
                                if dx != 0 or dy != 0:
                ...
                # draw main text
                p.setPen(text_color)
                p.drawText(pad, pad + ascent, text)
                p.end()
                cached = (pm, pad, ascent)
                # store in cache
                try:
                    self._text_pixmap_cache[key] = cached
                except Exception:
                    pass

            # blit cached pixmap - compute top-left from baseline coordinates
            pm, pad, ascent = cached
            top_left_x = int(x - pad)
            top_left_y = int(y - ascent - pad)
            painter.drawPixmap(top_left_x, top_left_y, pm)
            return
        except Exception:
            # fallback to original immediate draw path on any error
            pass
