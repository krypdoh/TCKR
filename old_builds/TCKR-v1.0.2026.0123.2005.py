...
def set_appbar(hwnd, height, rect):
    ...
    # CRITICAL: Determine the desired PHYSICAL appbar height while ensuring
    # we always request at least the actual physical window height.
    # `height` is interpreted as the desired physical pixels (caller should
    # convert logical->physical using the screen devicePixelRatio).
    ...
    if DEBUG_APPBAR: print(f"[APPBAR] Desired physical height requested: {height}px, Actual window height: {actual_window_height}px")

    # Ensure we request at least the actual window height (physical pixels)
    appbar_height = max(int(height), int(actual_window_height))
...
def get_settings():
    ...
    return {
        ...
        "websocket_incremental_interval_ms": 30,  # Process a batch of symbol rebuilds every N ms
        "websocket_incremental_max_per_tick": 6  # Max symbols rebuilt per timer tick
    }
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0123.2005")
    ...
    def setup_appbar_and_position(self):
        ...
        def after_removal():
            ...
            def setup_new_appbar():
                ...
                if DEBUG_APPBAR: print(f"[APPBAR] Registering PRIMARY ticker as appbar: logical_height={appbar_height}, screen={rect}")
                # Determine the screen object corresponding to this rect so we can
                # convert logical pixels to physical pixels using devicePixelRatioF.
                app = QtWidgets.QApplication.instance()
                screen = None
                ...
                desired_physical_height = int(round(appbar_height * dpr))
                if DEBUG_APPBAR: print(f"[APPBAR] Converted to physical height: {desired_physical_height}px (dpr={dpr})")
