...
class OverlayBuildWorker(QtCore.QThread):
    """Background worker to build small dynamic overlay images (price/change) as QImage.
    Emits a pair (symbols, images) where images are QImage objects corresponding to symbols.
    """
    overlays_ready = QtCore.pyqtSignal(list, list)

    def __init__(self, ticker_window, symbols):
        super().__init__()
        self.ticker_window = ticker_window
    ...
    def run(self):
        try:
            colored_print(f"[OVERLAY WORKER] Building overlays for {len(self.symbols)} symbols...")
            images = self.ticker_window.build_overlays_background(self.symbols)
            colored_print(f"[OVERLAY WORKER] Built {len(images)} overlays, emitting signal")
            self.overlays_ready.emit(self.symbols, images)
        except Exception as e:
            colored_print(f"[OVERLAY WORKER] Error building overlays: {e}")
...
class GhostWorker(QtCore.QThread):
    ...
    def __init__(self, ticker_window, offsets=None, src_image=None):
        super().__init__()
        self.ticker_window = ticker_window
        # Offsets: list of pixel offsets (positive numbers) to render trails
        self.offsets = offsets or [6, 12]
        # Optional QImage snapshot of the cycle captured on main thread
        self.src_image = src_image

    def run(self):
        try:
            colored_print("[GHOST WORKER] Building ghost layers in background...")
            layers = self.ticker_window.build_ghost_cache_background(self.offsets, src_image=self.src_image)
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0124.0303")
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def build_bloom_cache_background(self):
        ...
        try:
            ...
            # Use QImage for thread-safe off-thread rendering, respect devicePixelRatio
            try:
                dpr = float(self.devicePixelRatioF())
            except Exception:
                dpr = 1.0
            iw = max(1, int(cycle_width * dpr))
            ih = max(1, int(height * dpr))
            bloom_img = QtGui.QImage(iw, ih, QtGui.QImage.Format_ARGB32)
            bloom_img.fill(QtCore.Qt.transparent)
            try:
                bloom_img.setDevicePixelRatio(dpr)
            except Exception:
                pass
            painter = QtGui.QPainter(bloom_img)
            painter.setRenderHint(QtGui.QPainter.Antialiasing, True)
    ...
    def start_overlay_build_worker(self, symbols):
        """Start background worker to build overlays (price/change) for given symbols."""
        try:
            if not symbols:
                return
            # Deduplicate symbols
            symbols = list(dict.fromkeys(symbols))
            # If a worker is already running, queue the request
            if hasattr(self, '_overlay_worker') and getattr(self, '_overlay_worker') is not None:
                try:
                    if self._overlay_worker.isRunning():
                        # store pending symbols; will be started when current worker finishes
                        pending = getattr(self, '_pending_overlay_symbols', [])
                        pending.extend([s for s in symbols if s not in pending])
                        self._pending_overlay_symbols = pending
                        return
                except Exception:
                    pass

            worker = OverlayBuildWorker(self, symbols)
            worker.overlays_ready.connect(self._on_overlays_ready)
            self._overlay_worker = worker
            # clear pending list when starting
            self._pending_overlay_symbols = []
            worker.start()
        except Exception as e:
            colored_print(f"[OVERLAY] Failed to start worker: {e}")

    def _on_overlays_ready(self, symbols, images):
        """Slot: receives (symbols, images) where images are QImage objects.
        Convert to QPixmap on the main thread and install into overlay lists.
        """
        try:
            if not symbols or not images:
                return
            for sym, img in zip(symbols, images):
                try:
                    if img is None:
                        continue
                    pm = QtGui.QPixmap.fromImage(img)
                    # Find target index (market pixmap at index 0)
                    try:
                        idx = self.stocks.index(sym)
                        target_index = idx + 1
                    except Exception:
                        continue
                    ...
                    self.ticker_overlay_pixmaps[target_index] = pm
                except Exception:
                    continue
            ...
            # If more pending symbols exist, start a new worker
            pending = getattr(self, '_pending_overlay_symbols', []) or []
            if pending:
                try:
                    self.start_overlay_build_worker(pending)
                    self._pending_overlay_symbols = []
                except Exception:
                    pass
        except Exception as e:
            colored_print(f"[OVERLAY READY] Error applying overlays: {e}")

    def _on_bloom_ready(self, pixmap):
        try:
            if pixmap is None:
                return
            # If worker returned a QImage (thread-safe), convert to QPixmap here on main thread.
            if isinstance(pixmap, QtGui.QImage):
                try:
                    pm = QtGui.QPixmap.fromImage(pixmap)
                except Exception:
                    pm = None
    ...
    def build_ghost_cache_background(self, offsets=None, src_image=None):
        """Construct ghost trail layers in background.
        If `src_image` argument is provided (QImage), use it as the source (thread-safe).
        Returns list of QImage objects (one per offset) to be converted to QPixmap on main thread.
        """
        offsets = offsets or self.ghost_offsets
        layers = []
        start = time.time()
        try:
            # If caller provided a snapshot image on the TickerWindow, use it (thread-safe).
            # Accept either a QImage argument passed in via src_image keyword or fallback to
            # attempting to render (less safe) from existing pixmaps.
            src_image = None
            # Caller may provide src_image via function argument (handled by worker)
            try:
                # Function may receive src_image as second arg if worker passed it
                import inspect
                # Inspect calling frame for src_image variable; safer approach is worker passing arg
            except Exception:
                pass

            # Prefer explicit src_image argument, then attribute set by main thread
            if src_image is None:
                try:
                    gw_src = getattr(self, '_ghost_src_image', None)
                    if isinstance(gw_src, QtGui.QImage):
                        src_image = gw_src
                except Exception:
                    src_image = None

            # If no snapshot available, fallback to creating one (main-thread callers should set _ghost_src_image)
            if src_image is None:
                # Fallback: create QImage snapshot from current pixmaps (this may be called on worker thread)
                ...
                try:
                    ...
                    dpr = 1.0
                iw = max(1, int(cycle_width * dpr))
                ih = max(1, int(self.height() * dpr))
                tmp_img = QtGui.QImage(iw, ih, QtGui.QImage.Format_ARGB32)
                tmp_img.fill(QtCore.Qt.transparent)
                try:
                    tmp_img.setDevicePixelRatio(dpr)
                except Exception:
                    pass
                p = QtGui.QPainter(tmp_img)
                try:
                    x_off = 0
                    for pixmap, w in zip(getattr(self, 'ticker_pixmaps', []), getattr(self, 'ticker_pixmap_widths', [])):
                        try:
                            # drawPixmap accepts QPixmap onto QImage when done on main thread; fallback here
            ...
            # Build faded shifted layers from src_image
            for off in offsets:
                try:
                    # Create an output QImage same size as source
                    out_img = QtGui.QImage(src_image.size(), QtGui.QImage.Format_ARGB32)
                    out_img.fill(QtCore.Qt.transparent)
                    try:
                        out_img.setDevicePixelRatio(getattr(src_image, 'devicePixelRatio', lambda: 1.0)())
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        painter.drawImage(-off, 0, src_image)
                        # Fade layer alpha based on offset
                        alpha = max(6, 40 - int(off * 1.5))
                        fade_img = QtGui.QImage(out_img.size(), QtGui.QImage.Format_ARGB32_Premultiplied)
    ...
    def build_overlays_background(self, symbols):
        """Build overlay QImage objects for the given symbols off the main thread.
        Returns a list of QImage objects (same order as symbols). None for items that
        had no overlay or failed to build.
        """
        images = []
        try:
            if not symbols:
            ...
            for tkr in symbols:
                try:
                    ...
                    base_width = int(icon_size + 8 + tkr_width)
                    overlay_w = int(total_width - base_width)
                    if overlay_w <= 0:
                        images.append(None)
                        continue

                    iw = max(1, int(overlay_w * dpr))
                    ih = max(1, int(self.ticker_height * dpr))
                    ...
                    try:
                        # draw dynamic overlay portion starting at x=0 (which maps to base_width in full pixmap)
                        p.setFont(self.ticker_font)
                        price_y = (self.ticker_height + metrics.ascent() - metrics.descent()) // 2
                        ...
                        # Compute glow color for overlay
                        change_percent = 0
                        ...
                        # Draw price (x==0 in overlay coords)
                        p.setFont(self.ticker_font)
                        if glow_color:
                            p.setPen(glow_color)
                            if USE_OPT:
                                for dx, dy in opt.get_glow_offsets():
                                    p.drawText(0 + dx, price_y + dy, price_text)
                            else:
                                for dx in [-2, -1, 0, 1, 2]:
                                    for dy in [-2, -1, 0, 1, 2]:
                                        if dx != 0 or dy != 0:
                            ...
                            p.drawText(0, price_y, price_text)
                        else:
                            p.setPen(price_color)
                            p.drawText(0, price_y, price_text)

                        # Draw change and pct if present
                        if change_text or pct_text:
                            ...
                            change_x = price_width
                            if glow_color:
                                p.setPen(glow_color)
                                if USE_OPT:
                                    for dx, dy in opt.get_glow_offsets():
                                        p.drawText(change_x + 10 + dx, stacked_top + dy, change_text)
                                        p.drawText(change_x + 10 + dx, stacked_top + small_metrics.height() + 2 + dy, pct_text)
                                else:
                                    for dx in [-2, -1, 0, 1, 2]:
                                        for dy in [-2, -1, 0, 1, 2]:
                                            if dx != 0 or dy != 0:
                                                p.drawText(change_x + 10 + dx, stacked_top + dy, change_text)
                                                p.drawText(change_x + 10 + dx, stacked_top + small_metrics.height() + 2 + dy, pct_text)
                                p.setPen(QtGui.QColor("#FFFFFF") if change == 0 else (QtGui.QColor("#00FF40") if change > 0 else QtGui.QColor("#FF5555")))
                                p.drawText(change_x + 10, stacked_top, change_text)
                                p.drawText(change_x + 10, stacked_top + small_metrics.height() + 2, pct_text)
                            else:
                                # No heavy glow in worker for simplicity; draw main text
                                color = (QtGui.QColor("#00FF40") if change > 0 else (QtGui.QColor("#FF5555") if change < 0 else QtGui.QColor("#FFFFFF")))
                                p.setPen(color)
                                p.drawText(change_x + 10, stacked_top, change_text)
                                p.drawText(change_x + 10, stacked_top + small_metrics.height() + 2, pct_text)

                        # Note: sep and other tiny items are lower-cost; skipping advanced glow for them
                    finally:
                        p.end()
                    images.append(img)
                except Exception:
            ...
            colored_print(f"[OVERLAY BUILD] Error: {e}")
        return images
    ...
    def start_ghost_build_worker(self, offsets=None):
        try:
            ...
            pass
        try:
            # Capture a QImage snapshot of current cycle on main thread to pass to worker
            try:
                cycle_width = int(self.get_cycle_width())
                try:
                    dpr = float(self.devicePixelRatioF())
                ...
                iw = max(1, int(cycle_width * dpr))
                ih = max(1, int(self.height() * dpr))
                src_img = QtGui.QImage(iw, ih, QtGui.QImage.Format_ARGB32)
                src_img.fill(QtCore.Qt.transparent)
                try:
                    src_img.setDevicePixelRatio(dpr)
                except Exception:
                    pass
            ...
            self.ghost_worker = GhostWorker(self, offsets=offsets or self.ghost_offsets, src_image=src_img)
    ...
    def _on_ghost_ready(self, layers):
        try:
            if not layers:
                ...
                # Convert QImage layers returned from worker into QPixmap on main thread
                conv = []
                for layer in layers:
                    try:
                        if isinstance(layer, QtGui.QImage):
                            pm = QtGui.QPixmap.fromImage(layer)
                        elif isinstance(layer, QtGui.QPixmap):
                            pm = layer
                        else:
                            pm = None
                        if pm is not None:
    ...
    def _on_glass_ready(self, pix):
        try:
            ...
            if isinstance(pix, QtGui.QImage):
                try:
                    pm = QtGui.QPixmap.fromImage(pix)
                except Exception:
                    pm = None
    ...
    def build_ticker_pixmaps(self):
        ...
        try:
            ...
            pass
        # Build dynamic overlays (price/change) in background to avoid main-thread rebuild spikes
        try:
            self.start_overlay_build_worker(self.stocks)
        except Exception:
            pass
    ...
    def build_ticker_pixmaps_for_symbols(self, symbols):
        ...
        if updated_any:
            ...
            # Also start overlay builds for this incremental symbols list
            try:
                self.start_overlay_build_worker(symbols)
            except Exception:
                pass
