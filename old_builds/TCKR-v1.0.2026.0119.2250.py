    ...
    def subscribe_symbols(self, symbols):
        ...
        try:
            ...
            if self.subscribed_symbols:
                ...
                unsubscribe_msg = {
                    'type': 'unsubscribe',
                    'symbol': ','.join(self.subscribed_symbols)
                }
                self.ws.send(json.dumps(unsubscribe_msg))
                colored_print(f"[WEBSOCKET] 📤 Unsubscribed from {old_count} symbols")

            # Subscribe to new symbols
            if finnhub_symbols:
                subscribe_msg = {
                    'type': 'subscribe',
                    'symbol': ','.join(finnhub_symbols)
                }
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_websocket_status(self):
        ...
        if not hasattr(self.ticker_window, 'websocket_client') or not self.ticker_window.websocket_client:
        ...
        client = self.ticker_window.websocket_client
        status = client.get_debug_status()

        market_open = getattr(self.ticker_window, '_market_is_open', False)
        finnhub_count = len([s for s in self.ticker_window.stocks if not s.startswith('^')])
        ...
        QtWidgets.QMessageBox.information(self.ticker_window, "WebSocket Status", status_msg)
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def check_websocket_cost_startup(self):
        """Check for websocket cost warnings on startup"""
    ...
    def update_price_from_websocket(self, symbol, price, timestamp):
        ...
        # Only log every 5th update to avoid spam, but always log significant price changes
        old_price = self.prices.get(symbol, (None, None))[0]
        should_log = False
        if old_price is None:
            should_log = True  # First price update
        elif hasattr(self, '_websocket_update_counter'):
            self._websocket_update_counter += 1
            if self._websocket_update_counter % 5 == 0:
                should_log = True  # Every 5th update
            elif abs(price - old_price) / old_price > 0.001:  # >0.1% change
                should_log = True  # Significant price change
        else:
            self._websocket_update_counter = 1
            should_log = True  # First update
        ...
        # Create a price update dict with just this symbol
        websocket_prices = {symbol: (price, self.prices.get(symbol, (None, None))[1])}

        # Process the update using existing price processing logic
        self.on_prices_inplace_fetched(websocket_prices)
