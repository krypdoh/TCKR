            ...
            if hasattr(self, 'tray_icon') and self.tray_icon and self == getattr(self.tray_icon, 'primary_ticker', None):
                for ticker in self.tray_icon.ticker_windows:
                    if ticker is self:
                        continue
                    try:
                        # Use the same deferred processing flow as primary: set pending update
                        ticker._pending_price_update = prices.copy()
                        # Ensure visual state flags match
                        ticker.loading = False
                        ticker.bloom_cache_valid = False
                        ticker.last_api_update_time = now
                        # Schedule processing on the ticker's main thread
                        QtCore.QTimer.singleShot(0, ticker._process_price_update_deferred)
                        print(f"[PRIMARY UPDATE] Distributed inplace prices to secondary ticker: {getattr(ticker, 'objectName', lambda: 'ticker')()}")
                    except Exception:
                        # Don't let one failing secondary block others
                        pass
        except Exception:
            pass
