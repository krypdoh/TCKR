...
class FinnhubWebSocketClient:
    ...
    def process_buffered_updates(self):
        ...
        # Process each buffered symbol with rate limiting
        for symbol, (price, timestamp) in list(self.price_buffer.items()):
        ...
        if updates_sent > 0 or symbols_skipped > 0:
            colored_print(f"[WEBSOCKET] 📈 Batch complete: {updates_sent} sent, {symbols_skipped} skipped (rate/threshold) | Total: {self.price_updates_processed} updates")
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, is_secondary=False):
        ...
        if self.websocket_enabled:
            ...
            if api_key:
                ...
                # ZERO-INTERRUPTION MODE: No batch timer!
                # WebSocket messages accumulate silently in buffer
                # Process buffer + rebuild visuals ONLY every 10 seconds
                # This gives maximum smoothness with acceptable 10s update latency
                ...
                self.websocket_visual_timer.start(10000)  # Process + rebuild every 10 seconds
                self.websocket_visual_timer.setTimerType(QtCore.Qt.VeryCoarseTimer)
                self.websocket_has_updates = False  # Track if we received updates since last rebuild
                colored_print("[WEBSOCKET] 🎨 Zero-interruption mode: Process buffer + rebuild every 10s")
    ...
    def refresh_websocket_visuals(self):
        ...
        # Step 1: Process buffered WebSocket updates first
        if hasattr(self, 'websocket_client') and self.websocket_client:
            self.websocket_client.process_buffered_updates()
        
        # Step 2: Check if we have updates to display
        if not hasattr(self, 'websocket_has_updates') or not self.websocket_has_updates:
            return  # No updates since last refresh, skip rebuild
        
        # Step 3: Rebuild ticker with latest prices
        if not self.loading and self.stocks:
            ...
            # Log every 10 refreshes to avoid spam
            if self._websocket_refresh_count % 10 == 0:
                colored_print(f"[WEBSOCKET] 🎨 Refresh #{self._websocket_refresh_count}: processed buffer + rebuilt ticker")
    
    def refresh_websocket_visuals_deferred(self):
        """Deferred visual refresh - schedules rebuild for next idle period"""
        if not hasattr(self, 'websocket_has_updates') or not self.websocket_has_updates:
            return  # No updates since last refresh, skip
