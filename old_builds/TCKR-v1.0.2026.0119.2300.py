...
def fetch_all_stock_prices_with_429(tickers, api_key, api_key_2=None):
    ...
    if cache_key in fetch_all_stock_prices_with_429._cache:
        ...
        if current_time - cached_time < 60:
            colored_print(f"[API CACHE] Using cached results for {len(tickers)} tickers (age: {current_time - cached_time:.1f}s)")
            return cached_prices, False
        else:
            colored_print(f"[API CACHE] Cache expired for {len(tickers)} tickers (age: {current_time - cached_time:.1f}s) - fetching fresh data")
...
class FinnhubWebSocketClient:
    ...
    def connect(self):
        ...
        try:
            ...
            def on_open(ws):
                ...
                # Subscribe to symbols will be called after connection
            ...
            for trade in trades:
                ...
                if symbol and price:
                    ...
                    QtCore.QMetaObject.invokeMethod(
                        self.window,
                        "update_price_from_websocket",
                        QtCore.Qt.QueuedConnection,
                        QtCore.Q_ARG(str, symbol),
                        QtCore.Q_ARG(float, price),
                        QtCore.Q_ARG(float, timestamp)
                    )

            if trades_processed > 0:
                # Log every 10 trades to avoid spam
                if self.price_updates_processed % 10 == 0:
                    colored_print(f"[WEBSOCKET] 📈 Processed {trades_processed} trades | Total: {self.price_updates_processed} updates, {self.messages_received} messages")
        elif msg_type == 'ping':
            # Respond to ping
            if self.ws:
                try:
    ...
    def subscribe_symbols(self, symbols):
        ...
        if not self.connected or not self.ws:
            colored_print(f"[WEBSOCKET] ❌ Cannot subscribe - not connected or no websocket")
            return
        ...
        current_symbols = set(finnhub_symbols)
        colored_print(f"[WEBSOCKET] Current subscribed: {len(self.subscribed_symbols)}, New: {len(current_symbols)}")

        if current_symbols == self.subscribed_symbols:
            colored_print(f"[WEBSOCKET] ✅ Already subscribed to these symbols")
            return  # No change needed

        try:
            # Unsubscribe from old symbols (Finnhub expects one symbol per message)
            if self.subscribed_symbols:
                old_syms = list(self.subscribed_symbols)
                for sym in old_syms:
                    unsubscribe_msg = {'type': 'unsubscribe', 'symbol': sym}
                    self.ws.send(json.dumps(unsubscribe_msg))
                colored_print(f"[WEBSOCKET] 📤 Unsubscribed from {len(old_syms)} symbols")

            # Subscribe to new symbols (one message per symbol)
            if finnhub_symbols:
                for sym in finnhub_symbols:
                    subscribe_msg = {'type': 'subscribe', 'symbol': sym}
                    self.ws.send(json.dumps(subscribe_msg))
                colored_print(f"[WEBSOCKET] 📥 Subscribed to real-time data for {len(finnhub_symbols)} symbols: {', '.join(finnhub_symbols[:5])}{'...' if len(finnhub_symbols) > 5 else ''}")
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0119.1810")
    ...
    def setup_appbar_and_position(self):
        ...
        def after_removal():
            ...
            def setup_new_appbar():
                ...
                # Calculate appbar height - double if second ticker is enabled
                settings = get_settings()
                appbar_height = self.ticker_height
                if settings.get("enable_second_ticker", False):
                    appbar_height = self.ticker_height * 2
                    if DEBUG_APPBAR: print(f"[APPBAR] Second ticker enabled - reserving double height: {appbar_height}px")

                if DEBUG_APPBAR: print(f"[APPBAR] Registering PRIMARY ticker as appbar: height={appbar_height}, screen={rect}")
                actual_top = set_appbar(int(self.winId()), appbar_height, rect)
    ...
    def update_price_from_websocket(self, symbol, price, timestamp):
        ...
        for ticker in targets:
            ...
            if old_price is None:
                ...
                if ticker._websocket_update_counter % 5 == 0:
                    should_log = True  # Every 5th update
                elif old_price and abs(price - old_price) / old_price > 0.001:  # >0.1% change
                    should_log = True
            else:
                ticker._websocket_update_counter = 1
                should_log = True
            ...
            # Update price dict immediately (fast)
            ticker.prices[symbol] = (price, prev_close)
            ticker.loading = False
            ticker.bloom_cache_valid = False
            ticker.last_api_update_time = timestamp

            # Batch updates - only rebuild ticker text periodically (every 200ms max, or every 10 updates)
            # This prevents performance issues with high-frequency websocket updates
            if not hasattr(ticker, '_websocket_batch_timer'):
                ticker._websocket_batch_timer = QtCore.QTimer()
                ticker._websocket_batch_timer.setSingleShot(True)
                ticker._websocket_batch_timer.timeout.connect(lambda t=ticker: t.build_ticker_text(reset_scroll=False))
                ticker._websocket_batch_count = 0
            
            ticker._websocket_batch_count = getattr(ticker, '_websocket_batch_count', 0) + 1
            
            # Rebuild immediately if we've accumulated 10 updates (prevents indefinite delay)
            if ticker._websocket_batch_count >= 10:
                ticker._websocket_batch_timer.stop()
                ticker.build_ticker_text(reset_scroll=False)
                ticker._websocket_batch_count = 0
            else:
                # Restart timer - will trigger rebuild after 200ms of no updates
                ticker._websocket_batch_timer.stop()
                ticker._websocket_batch_timer.start(200)
