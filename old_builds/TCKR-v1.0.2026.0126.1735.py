...
class EffectsPreviewWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Prefer using the same stock/display name and font as the main TextTicker window
        sample_symbol = "TEST"
        sample_price = "123.45"
        try:
            ...
            pass
        # Prefer the parent's chosen ticker font and make it a bit larger for preview
        parent_font = None
        # Prefer the main ticker window's font (registered at get_ticker_icon._window_instance)
        try:
            main_win = getattr(get_ticker_icon, '_window_instance', None)
            ...
            parent_font = None
        # Fall back to settings dialog's parent font if main window not available
        ...
        if parent_font is None:
            parent_font = QtGui.QFont("Consolas", 18)
        base_sz = parent_font.pointSize()
        if base_sz <= 0:
            base_sz = 18
        # Increase preview size a bit more for clarity
        parent_font.setPointSize(max(10, int(base_sz * 1.35)))
        font = parent_font
        painter.setFont(font)

        # Compute layout so symbol and price don't overlap in preview
        available_width = self.width() - 20  # 10px padding each side
        metrics = QtGui.QFontMetrics(font)
        sym_w = metrics.horizontalAdvance(sample_symbol)
        price_w = metrics.horizontalAdvance(sample_price)
        gap = 12
        # If both don't fit, reduce font size until they fit or minimum reached
        cur_font = QtGui.QFont(font)
        cur_sz = cur_font.pointSize()
        while (sym_w + price_w + gap) > available_width and cur_sz > 10:
            cur_sz -= 1
            cur_font.setPointSize(cur_sz)
            metrics = QtGui.QFontMetrics(cur_font)
            sym_w = metrics.horizontalAdvance(sample_symbol)
            price_w = metrics.horizontalAdvance(sample_price)
        # Final positions: symbol left, price right-aligned if needed
        symbol_x = 10
        if symbol_x + sym_w + gap + price_w <= self.width() - 10:
            price_x = symbol_x + sym_w + gap
        else:
            price_x = self.width() - 10 - price_w
            # If price would overlap symbol, move symbol left (shrink) -- already handled by font shrink
        painter.setFont(cur_font)
        fm = QtGui.QFontMetrics(cur_font)
        y = (self.height() + fm.ascent() - fm.descent()) // 2

        # Draw ghost layers (simplified)
        if self.ghost_enabled and self.ghost_intensity > 0:
            ...
            for i, off in enumerate(offsets[::-1]):
                ...
                # symbol ghost (blue-ish)
                gcolor = QtGui.QColor(0, 179, 255, alpha)
                painter.setPen(gcolor)
                painter.drawText(int(symbol_x + off), int(y + off), sample_symbol)
                # price ghost (green-ish)
                gcolor2 = QtGui.QColor(0, 255, 64, alpha)
                painter.setPen(gcolor2)
                painter.drawText(int(price_x + off), int(y + off), sample_price)

        # Draw bloom (simplified radial) behind price if enabled
        if self.bloom_enabled and self.bloom_intensity > 0:
            bf = calculate_bloom_factor(self.bloom_intensity)
            # Use current font metrics and computed positions for bloom
            metrics = QtGui.QFontMetrics(cur_font)
            # Symbol bloom (blue)
            sym_x = int(symbol_x)
            sym_w = metrics.horizontalAdvance(sample_symbol)
            sym_rect = QtCore.QRect(sym_x, 0, sym_w, int(self.height()))
            sym_center = sym_rect.center()
            sym_r = max(sym_rect.width(), sym_rect.height()) * 0.6
            sym_grad = QtGui.QRadialGradient(sym_center.x(), sym_center.y(), sym_r)
            sym_alpha = int(60 * bf)
            sym_grad.setColorAt(0, QtGui.QColor(0, 179, 255, min(255, sym_alpha)))
            sym_grad.setColorAt(1, QtGui.QColor(0, 0, 0, 0))

            # Price bloom (green)
            price_x = int(price_x)
            price_w = metrics.horizontalAdvance(sample_price)
            price_rect = QtCore.QRect(price_x, 0, price_w, int(self.height()))
            price_center = price_rect.center()
            price_r = max(price_rect.width(), price_rect.height()) * 0.6
            price_grad = QtGui.QRadialGradient(price_center.x(), price_center.y(), price_r)
            price_alpha = int(60 * bf)
            price_grad.setColorAt(0, QtGui.QColor(0, 255, 64, min(255, price_alpha)))
            price_grad.setColorAt(1, QtGui.QColor(0, 0, 0, 0))

            # Draw additive blooms
            painter.save()
            ...
            painter.setBrush(QtGui.QBrush(price_grad))
            painter.drawEllipse(int(price_center.x() - price_r), int(price_center.y() - price_r), int(price_r * 2), int(price_r * 2))
            painter.restore()

        # Draw main text on top
        painter.setPen(QtGui.QColor("#00B3FF"))
        painter.drawText(int(symbol_x), int(y), sample_symbol)
        painter.setPen(QtGui.QColor("#00FF40"))
        painter.drawText(int(price_x), int(y), sample_price)
        painter.end()
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        # Motion Blur/Ghosting with inline intensity control
        ghost_row = QtWidgets.QHBoxLayout()
        ...
        self.led_ghost_intensity_spin.setSuffix("%")
        ...
        self.led_ghost_intensity_spin.setMaximumWidth(120)
        ghost_row.addWidget(self.led_ghost_intensity_spin)
        ghost_row.addStretch()
        effects_layout.addLayout(ghost_row)
        ...
        effects_layout.addLayout(preview_layout)
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0126.1735")
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, is_secondary=False):
        ...
        if sys.platform == "win32":
            ...
            # Debug: show a visible magenta bar during ghost render passes to verify drawing
            # Toggle this to False to disable the overlay once debugging is complete
            self._ghost_debug_visual = True
    ...
    def build_ticker_pixmaps(self):
        ...
        # Log center and small-area average alpha to ensure tint applied where text exists
        try:
            img = market_ghost.toImage()
            w = img.width(); h = img.height()
            cx, cy = w//2, h//2
            center_alpha = img.pixelColor(cx, cy).alpha()
            # sample a 3x3 area around center for a simple average
            ssum = 0; scount = 0
    ...
    def apply_ghosting_effect(self, painter, width, height, settings, behind_main=False):
        ...
        # --- Debug: lightweight, rate-limited logging to help diagnose missing ghosting ---
        try:
            import time as _time_mod
            now = _time_mod.time()
            last = getattr(self, '_ghost_debug_last', 0)
            if now - last > 1.0:  # once per second max to avoid spamming
        ...
        for layer_idx, ghost_offset in enumerate(reversed(offsets)):
            ...
            # Rate-limited trace to show computed opacity and a sample ghost pixmap alpha
            try:
                import time as _time_mod
                now = _time_mod.time()
            ...
            # Rate-limited log of draws per layer for diagnostics
            try:
                import time as _time_mod
                now = _time_mod.time()
        ...
        # Log summary of draws to help determine whether ghost draw operations are happening
        try:
            import time as _time_mod
            now = _time_mod.time()
            ...
            if now - last > 1.0:
                ...
                # If we drew something but user can't see, render a visible debug rectangle
