...
def set_appbar(hwnd, height, rect):
    ...
    # Set the proposed rectangle spanning the full monitor width at the requested height.
    # Use left + width for the exclusive right edge (Windows RECT convention).
    ...
    # --- Correct Windows AppBar protocol ---
    # ABM_QUERYPOS: the shell adjusts abd.rc to respect already-registered AppBars.
    # For ABE_TOP the shell may push abd.rc.top downward if another AppBar is already there.
    # CRITICAL: do NOT overwrite abd.rc.top after this call.
    ...
    # Maintain the desired height from the (possibly adjusted) top.
    abd.rc.bottom = abd.rc.top + appbar_height

    # ABM_SETPOS: commit the negotiated rectangle. The shell automatically updates the
    # system work area — we must NOT call SPI_SETWORKAREA afterwards because that would
    # overwrite other programs' (e.g. taskbar) reserved space at the bottom.
    ...
    # work_area_top_offset tracks what was actually reserved (used by diagnostics below)
    work_area_top_offset = actual_reserved_height
    
    # Notify other applications that the work area has changed.
    # ABM_SETPOS updates the work area automatically, but broadcasting WM_SETTINGCHANGE
    # ensures apps that cache the work area refresh their layout.
    # SMTO_ABORTIFHUNG prevents hung windows from stalling startup.
    ...
    # Position the window at the negotiated AppBar coordinates.
    # abd.rc.top is what the shell approved after QUERYPOS/SETPOS — it may be greater
    # than rect.top() if another AppBar already occupies the very top edge.
    HWND_TOPMOST = -1
    SWP_NOACTIVATE = 0x0010
    SWP_SHOWWINDOW = 0x0040
    user32.SetWindowPos(hwnd, HWND_TOPMOST,
                       abd.rc.left, abd.rc.top,
                       abd.rc.right - abd.rc.left, actual_window_height,
                       SWP_SHOWWINDOW | SWP_NOACTIVATE)
    if DEBUG_APPBAR: print(f"[APPBAR] SetWindowPos: x={abd.rc.left}, y={abd.rc.top}, w={abd.rc.right - abd.rc.left}, h={actual_window_height}")

    return abd.rc.top  # Negotiated top — may differ from rect.top() if another AppBar is there
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.0.2026.0317.1000")
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def setup_appbar_and_position(self):
        ...
        # Check if space is already reserved at the top (from another LIVE ticker).
        # IMPORTANT: Only trust the work-area reservation when coordinate_with_other_tickers
        # actually found other TCKR windows.  If has_other_tickers is False, any existing
        # reservation is stale (left over from a previous run whose window is now gone) and
        # must be ignored so we register fresh as the primary ticker.
        user32 = ctypes.windll.user32
        work_area = wintypes.RECT()
        user32.SystemParametersInfoW(48, 0, ctypes.byref(work_area), 0)  # SPI_GETWORKAREA
        existing_reservation = work_area.top - rect.top()
        
        if existing_reservation > 0 and not has_other_tickers:
            # Stale reservation from a previous run - reset it so we start clean
            if DEBUG_APPBAR: print(f"[APPBAR] Stale work-area reservation ({existing_reservation}px) with no live tickers - clearing it")
            user32.SystemParametersInfoW(47, 0, None, 0x0002)  # SPI_SETWORKAREA, let Windows recalculate
            existing_reservation = 0
        ...
        def after_removal():
            ...
            def setup_new_appbar():
                ...
                # Position the window at the negotiated AppBar top. actual_top == rect.top() in
                # the normal case; it only differs when another AppBar already occupies that edge.
                self.setGeometry(rect.left(), actual_top, rect.width(), self.ticker_height)
                pos = self.pos()
                if DEBUG_APPBAR: print(f"[APPBAR] Position after setGeometry: x={pos.x()}, y={pos.y()}")
                if DEBUG_POSITIONING: print(f"[POSITIONING] Screen top: {rect.top()}, AppBar negotiated: {actual_top}, Qt pos: {pos.y()}")
    ...
    def move_to_selected_screen(self):
        ...
        if sys.platform == "win32":
            ...
            def after_cleanup():
                ...
                self.move(rect.left(), actual_top)
                self.resize(rect.width(), self.ticker_height)
                print(f"[SCREEN MOVE] Screen top: {rect.top()}, AppBar negotiated: {actual_top}")
