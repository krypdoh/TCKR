...
try:
    ...
    if not hasattr(websocket, "WebSocketApp"):
        raise AttributeError("Missing WebSocketApp (likely 'websocket' package, not 'websocket-client')")
    WEBSOCKET_AVAILABLE = True
    colored_print("[WEBSOCKET] ✅ websocket-client library available")
except Exception as e:
    WEBSOCKET_AVAILABLE = False
    colored_print(f"[WEBSOCKET] ❌ websocket-client not available or wrong package: {e}")
    colored_print("[WEBSOCKET] Install with 'pip install websocket-client' and uninstall 'websocket' if present")
...
def is_yahoo_symbol(symbol):
    return isinstance(symbol, str) and (symbol.startswith('^') or symbol.startswith('$'))
...
def ticker_display_name(symbol):
    if symbol in INDEX_DISPLAY_NAMES:
        return INDEX_DISPLAY_NAMES[symbol]
    if is_crypto_symbol(symbol):
        return normalize_yahoo_symbol(symbol)
    return symbol


def get_icon_lookup_candidates(symbol):
    if not isinstance(symbol, str):
        return []
    candidates = []
    upper_symbol = symbol.upper()
    normalized = normalize_yahoo_symbol(upper_symbol)

    for candidate in (upper_symbol, normalized):
...
def get_crypto_icon_candidates(symbol):
    if not isinstance(symbol, str):
        return []
    normalized = normalize_yahoo_symbol(symbol.upper())
    candidates = []
    if normalized.endswith('-USD'):
        base = normalized[:-4]
        if base:
...
def stock_sort_key(symbol):
    if not symbol:
        return (3, '')
    if symbol.startswith('$'):
        return (0, symbol.upper())
    if symbol.startswith('^'):
        return (1, symbol.upper())
    return (2, symbol.upper())
...
def set_appbar(hwnd, height, rect, start_y=None, device_scale=1.0):
    """Register hwnd as a top-edge AppBar.

    Parameters
    ----------
    hwnd         : Win32 window handle
    height       : desired reservation height in PHYSICAL pixels
    rect         : Qt QRect for the target monitor in LOGICAL pixels
    start_y      : override for the initial top edge in PHYSICAL pixels (used when a
                   foreign app already occupies the very top of the screen)
    device_scale : logical-to-physical scale factor (e.g. 2.0 for 200% DPI)

    Returns
    -------
    Negotiated window top in LOGICAL pixels (ready for Qt setGeometry/move).
    """
    ...
    # Get the actual physical window height from Win32 (authoritative source)
    ...
    # Convert Qt logical rect to physical pixels for the AppBar API
    phys_left  = int(round(rect.left()  * device_scale))
    phys_top   = int(round(rect.top()   * device_scale))
    phys_right = int(round((rect.left() + rect.width()) * device_scale))
    ...
    abd.rc.left  = phys_left
    abd.rc.top   = start_y if start_y is not None else phys_top
    abd.rc.right = phys_right
    abd.rc.bottom = abd.rc.top + appbar_height

    # Register as AppBar
    result = shell32.SHAppBarMessage(ABM_NEW, ctypes.byref(abd))
    if DEBUG_APPBAR: print(f"[APPBAR] ABM_NEW result={result}, proposed: left={abd.rc.left}, top={abd.rc.top}, right={abd.rc.right}, bottom={abd.rc.bottom}")

    # ABM_QUERYPOS: the shell adjusts abd.rc to respect already-registered AppBars.
    shell32.SHAppBarMessage(ABM_QUERYPOS, ctypes.byref(abd))
    if DEBUG_APPBAR: print(f"[APPBAR] After QUERYPOS: top={abd.rc.top}, bottom={abd.rc.bottom}")

    # Determine the window position and final reservation rectangle.
    # When start_y is provided (foreign app reserves top space without using AppBar API),
    # ABM_QUERYPOS may have pushed us back to y=0 since it can't see non-AppBar reservations.
    # We handle two sub-cases:
    #   A) shell pushed us to >= start_y  → foreign app IS a registered AppBar; use its result
    #   B) shell pushed us below start_y  → foreign app NOT registered AppBar; override
    if start_y is not None:
        if abd.rc.top >= start_y:
            # Case A: shell correctly placed us below the registered foreign AppBar
            window_top_phys = abd.rc.top
            abd.rc.bottom = abd.rc.top + appbar_height
        else:
            # Case B: shell pushed us to top-of-screen; it doesn't know about the foreign app.
            # Claim the ENTIRE top band (foreign + ours) so the work area is updated correctly,
            # but position the window at start_y (below the foreign app).
            abd.rc.top = phys_top          # screen physical top (0 for primary monitor)
            abd.rc.bottom = start_y + appbar_height  # foreign region + TCKR height
            window_top_phys = start_y      # window goes below the foreign app
            if DEBUG_APPBAR: print(f"[APPBAR] Case B: QUERYPOS pushed to {abd.rc.top}, overriding rect to 0-{abd.rc.bottom}, window at {window_top_phys}")
    else:
        # Normal case: use whatever QUERYPOS negotiated
        window_top_phys = abd.rc.top
        abd.rc.bottom = abd.rc.top + appbar_height

    shell32.SHAppBarMessage(ABM_SETPOS, ctypes.byref(abd))
    if DEBUG_APPBAR: print(f"[APPBAR] After SETPOS: top={abd.rc.top}, bottom={abd.rc.bottom}, height={abd.rc.bottom - abd.rc.top}")

    actual_reserved_height = abd.rc.bottom - abd.rc.top
    if actual_reserved_height < appbar_height:
        if DEBUG_APPBAR: colored_print(f"[APPBAR WARNING] Windows reserved {actual_reserved_height}px instead of {appbar_height}px — another AppBar may be present")

    HWND_BROADCAST = 0xFFFF
    WM_SETTINGCHANGE = 0x001A
    SMTO_ABORTIFHUNG = 0x0002
    SPI_SETWORKAREA_MSG = 47
    user32.SendMessageTimeoutW(HWND_BROADCAST, WM_SETTINGCHANGE,
                               SPI_SETWORKAREA_MSG, 0,
                               SMTO_ABORTIFHUNG, 150, None)
    if DEBUG_APPBAR: print(f"[APPBAR] Broadcasted WM_SETTINGCHANGE (work area changed)")

    # Position the window at the decided physical top (ignores abd.rc.top which may differ)
    HWND_TOPMOST = -1
    SWP_NOACTIVATE = 0x0010
    SWP_SHOWWINDOW = 0x0040
    user32.SetWindowPos(hwnd, HWND_TOPMOST,
                       phys_left, window_top_phys,
                       phys_right - phys_left, actual_window_height,
                       SWP_SHOWWINDOW | SWP_NOACTIVATE)
    if DEBUG_APPBAR: print(f"[APPBAR] SetWindowPos: x={phys_left}, y={window_top_phys}, w={phys_right - phys_left}, h={actual_window_height}")

    # Return the window top in LOGICAL pixels for Qt's setGeometry/move
    negotiated_top_logical = window_top_phys / device_scale if device_scale else window_top_phys
    return negotiated_top_logical

def force_window_to_top(hwnd, rect, height):
    """Force window to absolute top of screen, ignoring other docked apps.
    Uses Qt logical coordinates for positioning (rect, height) - converts to physical for Win32."""
    user32 = ctypes.windll.user32
    ...
    # Use Qt to move/resize (it handles logical->physical conversion internally)
    # Then set TOPMOST z-order without touching position or size
    import ctypes as _ct
    # HWND_TOPMOST with NOMOVE+NOSIZE just updates the z-order
    user32.SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0,
                        SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE)
    
    # Verify physical position after the z-order update
...
def load_performance_modules():
    ...
    # Load compiled ticker utils — prefer Cython (AOT, instant), fall back to Numba (JIT)
    try:
        import ticker_utils_cython as opt_module
        opt = opt_module
        USE_OPT = True
        # ticker_utils_cython prints its own load message on import
    except ImportError:
        try:
            import ticker_utils_numba as opt_module
            opt = opt_module
            USE_OPT = True
            # ticker_utils_numba prints its own load message on import
        except ImportError:
            USE_OPT = False
            colored_print("[PERF] No optimized ticker utils found. Build ticker_utils_cython.pyd with setup_cython.py, or place ticker_utils_numba.py here.")
...
def get_settings():
    ...
    return {
        ...
        "show_market_status": True,  # Show Market Open/Closed status ticker item
    }
...
class EffectsPreviewWidget(QtWidgets.QWidget):
    """Live preview widget showing the ticker's stacked layout with all visual effects applied."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(560, 74)
    ...
    def _spin_value(self, name, default=100):
        parent = self.parent()
        widget = getattr(parent, name, None)
    ...
    def paintEvent(self, event):
        ...
        # Transparency: 100 = fully opaque, 0 = fully transparent
        transparency = self._spin_value('transparency_spin', 100)
        bg_alpha = int(255 * transparency / 100)

        # Checkerboard hint when semi-transparent so user can see the effect
        if bg_alpha < 255:
            chk = 8
            for row in range(self.height() // chk + 1):
                for col in range(self.width() // chk + 1):
                    c = QtGui.QColor(70, 70, 70) if (row + col) % 2 == 0 else QtGui.QColor(45, 45, 45)
                    c.setAlpha(255 - bg_alpha)
                    painter.fillRect(col * chk, row * chk, chk, chk, c)

        # Background gradient
        bg = QtGui.QLinearGradient(0, 0, 0, self.height())
        bg.setColorAt(0, QtGui.QColor(15, 18, 22, bg_alpha))
        bg.setColorAt(1, QtGui.QColor(8, 10, 14, bg_alpha))
        painter.fillRect(0, 0, self.width(), self.height(), bg)

        # --- Sample data (uses live data when available) ---
        sample_symbol = "AAPL"
        sample_price = "173.50"
        sample_change = "+2.14"
        sample_pct = "+1.25%"
                ...
                if hasattr(source_win, 'prices') and first in getattr(source_win, 'prices', {}):
                    ...
                    if p is not None:
                        ...
                        if prev is not None and prev != 0:
                            ...
                            sample_pct = f"{sign}{pct:.2f}%"
        except Exception:
            pass

        # --- Read settings from parent dialog ---
        ...
        # --- Layout: SYMBOL | PRICE | [CHANGE / PCT stacked] | INDICATOR | SPARKLINE ---
        left_pad = 10
        right_pad = 10
        sparkline_w = 72 if show_sparklines else 0
        sparkline_gap = 6 if show_sparklines else 0
        ind_gap = 6   # gap between stacked text and indicator

        if show_sparklines and sparkline_position == 'left':
            spark_rect = QtCore.QRect(left_pad, 4, sparkline_w, self.height() - 8)
            ...
            spark_rect = QtCore.QRect(content_right + sparkline_gap, 4, sparkline_w, self.height() - 8)
        else:
            spark_rect = None
            content_left = left_pad
            content_right = self.width() - right_pad

        # --- Font sizing: auto-reduce to fit two stacked rows ---
        try:
            main_win = getattr(get_ticker_icon, '_window_instance', None)
            if main_win and hasattr(main_win, 'ticker_font') and main_win.ticker_font is not None:
                base_font = QtGui.QFont(main_win.ticker_font)
            else:
                base_font = QtGui.QFont("Consolas", 12)
        except Exception:
            base_font = QtGui.QFont("Consolas", 12)
        ...
        price_font = QtGui.QFont(base_font)
        change_font = QtGui.QFont(base_font)
        pfm = cfm = QtGui.QFontMetrics(price_font)
        for pt in range(14, 6, -1):
            price_pt = round(pt * 1.2)   # symbol+price 20% larger than change/pct tier
            price_font.setPointSize(price_pt)
            change_font.setPointSize(max(6, pt - 3))
            pfm = QtGui.QFontMetrics(price_font)
            cfm = QtGui.QFontMetrics(change_font)
            sym_w = pfm.horizontalAdvance(sample_symbol)
            price_w = pfm.horizontalAdvance(sample_price)
            change_w = cfm.horizontalAdvance(sample_change)
            pct_w = cfm.horizontalAdvance(sample_pct)
            stacked_block_w = max(change_w, pct_w) + ind_gap + 16
            total_w = sym_w + 10 + price_w + 10 + stacked_block_w
            stacked_h = cfm.height() * 2 + 2
            if total_w <= available_w and stacked_h <= available_h and pfm.height() <= available_h:
                break

        # Final metrics after font size is set
        sym_w = pfm.horizontalAdvance(sample_symbol)
        price_w = pfm.horizontalAdvance(sample_price)
        change_w = cfm.horizontalAdvance(sample_change)
        pct_w = cfm.horizontalAdvance(sample_pct)

        # Vertical: symbol+price centered as single line; change+pct stacked and centered
        sym_baseline = (self.height() + pfm.ascent() - pfm.descent()) // 2
        price_baseline = sym_baseline
        stacked_h = cfm.height() * 2 + 2
        stacked_top = (self.height() - stacked_h) // 2
        change_baseline = stacked_top + cfm.ascent()
        pct_baseline = stacked_top + cfm.height() + 2 + cfm.ascent()

        # Indicator: centered on the top stacked row (matches real ticker)
        ind_center_y = stacked_top + cfm.height() // 2

        # Horizontal positions
        symbol_x = content_left
        price_x = symbol_x + sym_w + 10
        stacked_x = price_x + price_w + 10
        indicator_x = stacked_x + change_w + ind_gap
        ...
        if self.ghost_enabled and self.ghost_intensity > 0:
            ...
            for i, off in enumerate(offsets[::-1]):
                ...
                painter.drawText(int(symbol_x + off), int(sym_baseline + off), sample_symbol)
                painter.setPen(gcolor2)
                painter.drawText(int(price_x + off), int(price_baseline + off), sample_price)
                painter.setFont(change_font)
                painter.setPen(gcolor2)
                painter.drawText(int(stacked_x + off), int(change_baseline + off), sample_change)
                painter.drawText(int(stacked_x + off), int(pct_baseline + off), sample_pct)

        # --- Bloom effect ---
        if self.bloom_enabled and self.bloom_intensity > 0:
            bf = calculate_bloom_factor(self.bloom_intensity)
            sym_rect = QtCore.QRect(int(symbol_x), 0, sym_w, self.height())
            val_rect = QtCore.QRect(int(price_x), 0, price_w + 10 + max(change_w, pct_w) + ind_gap + 20, self.height())
        ...
        # --- Sparkline ---
        if show_sparklines and spark_rect is not None:
            self._draw_sparkline(painter, spark_rect, value_color, sparkline_period)

        # --- Symbol (large, vertically centered) ---
        painter.setFont(price_font)
        self._draw_text_with_glow(painter, symbol_x, sym_baseline, sample_symbol, symbol_color, text_glow_enabled)

        # --- Price (large, vertically centered) ---
        self._draw_text_with_glow(painter, price_x, price_baseline, sample_price, value_color, text_glow_enabled)

        # --- Stacked change (top) and pct (bottom) ---
        painter.setFont(change_font)
        self._draw_text_with_glow(painter, stacked_x, change_baseline, sample_change, value_color, text_glow_enabled)
        self._draw_text_with_glow(painter, stacked_x, pct_baseline, sample_pct, value_color, text_glow_enabled)

        # --- Price indicator (right of change text, centered on top stacked row) ---
        self._draw_price_indicator(painter, indicator_style, int(indicator_x), ind_center_y, is_up, value_color)
    ...
    if os.path.exists(STOCKS_FILE):
        try:
            with open(STOCKS_FILE, "r") as f:
                stocks = json.load(f)
                stocks.sort(key=lambda s: stock_sort_key(s[0]))
                return stocks
        except Exception:
            pass
...
def fetch_yahoo_quote(ticker):
    ...
    try:
        ...
        encoded_ticker = url_quote(yahoo_symbol, safe='')
...
def fetch_all_stock_prices(tickers, api_key, api_key_2=None):
    ...
    Uses Yahoo Finance for index and crypto symbols (starting with ^ or $).
    """
    prices = {}
    
    # Separate Yahoo Finance tickers (^ indices, $ crypto) from Finnhub tickers
    yahoo_tickers = [t for t in tickers if is_yahoo_symbol(t)]
    finnhub_tickers = [t for t in tickers if not is_yahoo_symbol(t)]
    ...
    if yahoo_tickers:
        colored_print(f"[YAHOO API] Fetching {len(yahoo_tickers)} Yahoo symbols from Yahoo Finance")
...
def build_websocket_proxy_kwargs(settings):
    ...
    if not host:
        colored_print("[WEBSOCKET] Proxy enabled but proxy host is missing")
        return {}
    port = parsed.port
    if port is None:
    ...
    if parsed.username:
        proxy_kwargs["http_proxy_auth"] = (parsed.username, parsed.password or "")
    if parsed.scheme:
...
class FinnhubWebSocketClient:
    ...
    def __init__(self, api_key, window_instance):
        ...
        self.connection_state_log = []  # Log of {timestamp, state, reason, duration}
        self.error_log = []  # Log of {timestamp, error, thread}
        self.connection_attempts_details = []  # {attempt, duration, success, error}
        ...
        self.proxy_kwargs = build_websocket_proxy_kwargs(settings)
        self.sslopt = None
        if settings.get("use_cert") and settings.get("cert_file"):
            self.sslopt = {"ca_certs": settings["cert_file"]}
        if self.proxy_kwargs:
            proxy_type = self.proxy_kwargs.get("proxy_type", "http")
            host = self.proxy_kwargs.get("http_proxy_host")
            port = self.proxy_kwargs.get("http_proxy_port")
            colored_print(f"[WEBSOCKET] Proxy enabled for WebSocket: {proxy_type}://{host}:{port}")
    ...
    def connect(self):
        ...
        try:
            ...
            def on_open(ws):
                ...
                self._log_state(f"Connected via {'proxy' if self.proxy_kwargs else 'direct'}", connection_duration)
                colored_print(f"[WEBSOCKET] ✅ Connected to Finnhub real-time data (took {connection_duration:.1f}s)")
                if self.proxy_kwargs:
                    proxy_type = self.proxy_kwargs.get('proxy_type', 'http')
                    host = self.proxy_kwargs.get('http_proxy_host')
                    colored_print(f"[WEBSOCKET]    🌐 Via Proxy: {proxy_type}://{host}:{self.proxy_kwargs.get('http_proxy_port')}")
                colored_print(f"[WEBSOCKET] 📊 Status: Connected | Messages: {self.messages_received} | Updates: {self.price_updates_processed}")
                colored_print(f"[WEBSOCKET] 🧵 Thread: {threading.current_thread().name}")
                # Flush any queued subscriptions now that we're connected
                try:
                    if self.queued_subscriptions:
                        finnhub_symbols = [s for s in list(self.queued_subscriptions) if not is_yahoo_symbol(s)]
                        if finnhub_symbols:
                            sent = 0
            ...
            def on_close(ws, close_status_code, close_msg):
                import time
                current_time = time.time()
                lag_since_msg = (current_time - self.last_message_time) if self.last_message_time else None
                self._log_state(f"Closed ({close_status_code})", close_msg)
                self.connected = False
                self.connecting = False
                if self.connection_start_time:
                    ...
                    if lag_since_msg:
                        colored_print(f"[WEBSOCKET]    Last message {lag_since_msg:.1f}s ago, {self.messages_received} total msgs")
                    ...
                    if self.proxy_kwargs:
                        colored_print(f"[WEBSOCKET]    Was using proxy - may be timeout or proxy disconnect")
                else:
                    colored_print(f"[WEBSOCKET] 🔌 Connection closed (never connected): {close_status_code} - {close_msg}")

            def on_error(ws, error):
                import time
                error_str = str(error)
                self._log_error(error_str)
                ...
                colored_print(f"[WEBSOCKET] ❌ Connection error [{error_type}]: {error_str}")
                if self.proxy_kwargs:
                    colored_print(f"[WEBSOCKET]    Proxy: {self.proxy_kwargs.get('http_proxy_host')}:{self.proxy_kwargs.get('http_proxy_port')}")
                self.connected = False
                self.connecting = False
    ...
    def _run_websocket(self):
        ...
        while self.reconnect_attempts < self.max_reconnect_attempts:
            ...
            try:
                ...
                if self.sslopt:
                    run_kwargs["sslopt"] = self.sslopt
                if self.proxy_kwargs:
                    run_kwargs.update(self.proxy_kwargs)
                    proxy_info = f"{self.proxy_kwargs.get('proxy_type', 'http')}://{self.proxy_kwargs.get('http_proxy_host')}:{self.proxy_kwargs.get('http_proxy_port')}"
                    colored_print(f"[WEBSOCKET] 🔌 Attempt {self.reconnect_attempts + 1}: Connecting via proxy {proxy_info}...")
                else:
                    colored_print(f"[WEBSOCKET] 🔌 Attempt {self.reconnect_attempts + 1}: Connecting directly to Finnhub...")
                if self.ws is None or self._stop_requested:
                    colored_print("[WEBSOCKET] 🛑 Stop requested or ws cleared - exiting reconnect loop")
                    break
                ...
                if not self.connected:
                    ...
                    self.connection_attempts_details.append({
                    ...
                    if self.reconnect_attempts < self.max_reconnect_attempts:
                        ...
                        colored_print(f"[WEBSOCKET]    Attempt duration: {attempt_duration:.1f}s | Last error: {self.last_error}")
                        time.sleep(delay)
                    else:
                ...
                attempt_duration = time.time() - attempt_start if attempt_start else 0
                error_str = f"{type(e).__name__}: {str(e)}"
                self._log_error(error_str)
                colored_print(f"[WEBSOCKET] ❌ WebSocket thread error: {error_str}")
                colored_print(f"[WEBSOCKET]    Traceback: {traceback.format_exc()[:200]}")
                self.connection_attempts_details.append({
                    'attempt': self.reconnect_attempts,
                    'duration': attempt_duration,
                    'success': False,
                ...
                if self.reconnect_attempts < self.max_reconnect_attempts:
                    delay = self._get_reconnect_delay(self.reconnect_attempts)
                    colored_print(f"[WEBSOCKET] ⏳ Waiting {delay}s before retry...")
                    time.sleep(delay)
                else:
                    break
        self.connecting = False
    ...
    def process_buffered_updates(self, max_symbols=None):
        ...
        # Drain the ENTIRE buffer in one pass — price dict writes are O(1) and take
        # microseconds total. Only queue a pixmap rebuild when the formatted display
        # string (e.g. "125.13") actually changes, which avoids work for sub-penny
        # trades that produce no visible difference.
        symbols_to_process = list(self.price_buffer.keys())
        
        if symbols_to_process and self.verbose_update_logs:
            colored_print(f"[WEBSOCKET] 🔄 Processing {buffer_size} buffered symbols (full drain)...")
        ...
        # Process each symbol — no per-symbol time gate needed; the visual refresh
        # timer itself controls how often we drain the buffer.
        for symbol in symbols_to_process:
            ...
            if hasattr(self.window, 'tray_icon') and self.window.tray_icon:
                ...
                for ticker in targets:
                    if hasattr(ticker, 'stocks') and symbol in ticker.stocks:
                        ...
                        fallback_active = getattr(ticker, '_ws_prev_close_fallback_active', None)
                        if fallback_active is None:
                            fallback_active = set()
                            ticker._ws_prev_close_fallback_active = fallback_active
                        if prev_close is None and old_price is not None and old_price > 0:
                            prev_close = old_price
                            if symbol not in fallback_active:
                                colored_print(f"[WEBSOCKET] 🟨 TEMP-BASELINE {symbol}: using last live price until prev_close backfills")
                                fallback_active.add(symbol)
                        elif prev_close is not None and symbol in fallback_active:
                            fallback_active.discard(symbol)
                        # Update price immediately
                        ticker.prices[symbol] = (price, prev_close)
                        ticker.bloom_cache_valid = False
                        
                        # Only flag display change if the formatted price string changed —
                        # sub-penny trades produce no visible difference, skip the rebuild
                        if old_price is None or f"{price:.2f}" != f"{old_price:.2f}":
            ...
            updates_sent += 1
            # Only queue a pixmap rebuild if the displayed text actually changed
            if display_changed and symbol not in updated_symbols:
    ...
    def subscribe_symbols(self, symbols):
        ...
        # Filter out Yahoo Finance symbols (^ indices, $ crypto) as they don't support websockets
        finnhub_symbols = [s for s in symbols if not is_yahoo_symbol(s)]
        colored_print(f"[WEBSOCKET] Filtered to {len(finnhub_symbols)} Finnhub symbols")

        if not finnhub_symbols:
            colored_print(f"[WEBSOCKET] 📊 No Finnhub symbols to subscribe to (all symbols are Yahoo Finance)")
            return
    ...
    def disconnect(self):
        ...
        # Signal the reconnect loop to stop before clearing self.ws
    ...
    def get_debug_status(self):
        ...
        if hasattr(self, 'connection_thread') and self.connection_thread:
            thread_alive = self.connection_thread.is_alive()
        ...
        return {
            ...
            'using_proxy': bool(self.proxy_kwargs),
            'proxy_type': self.proxy_kwargs.get('proxy_type') if self.proxy_kwargs else None,
    ...
    def log_status_summary(self):
        ...
        if status['connected']:
                lag = status['time_since_last_message']
                thread_status = "🧵 alive" if status['thread_alive'] else "🧵 DEAD"
                proxy_status = f" via {status['proxy_type']}" if status['using_proxy'] else " (direct)"
                colored_print(f"[WEBSOCKET] 📊 Status: ✅ Connected{proxy_status} {thread_status}")
                colored_print(f"[WEBSOCKET]    {status['subscribed_symbols']} symbols | {status['messages_received']} msgs | {status['price_updates_processed']} updates | {status['connection_duration']:.1f}s | lag {lag:.1f}s")
        else:
            thread_status = "🧵 alive" if status['thread_alive'] else "🧵 dead"
            proxy_status = f" (was via {status['proxy_type']})" if status['using_proxy'] else ""
            colored_print(f"[WEBSOCKET] 📊 Status: ❌ Disconnected {thread_status}{proxy_status}")
            colored_print(f"[WEBSOCKET]    {status['messages_received']} total msgs | {status['price_updates_processed']} updates | {status['reconnection_count']} reconnects | attempt {status['reconnect_attempts']}/{self.max_reconnect_attempts}")
            if status['last_error']:
                colored_print(f"[WEBSOCKET]    Last error: {status['last_error']}")
    
    def _log_state(self, new_state, reason="", duration=0):
        ...
        if len(self.connection_state_log) > 50:
            self.connection_state_log = self.connection_state_log[-50:]
    ...
    def _log_detailed_stats(self):
        ...
        colored_print("[WEBSOCKET] 📋 === Detailed Diagnostics ===")
        colored_print(f"[WEBSOCKET] State transitions: {len(self.connection_state_log)}")
        if self.connection_state_log:
            for i, entry in enumerate(self.connection_state_log[-5:]):
                colored_print(f"[WEBSOCKET]   {i}: {entry['state']} - {entry['reason']} ({entry['duration']:.1f}s)")
        colored_print(f"[WEBSOCKET] Errors recorded: {len(self.error_log)}")
        if self.error_log:
            for i, entry in enumerate(self.error_log[-5:]):
                colored_print(f"[WEBSOCKET]   {i}: {entry['error']} (thread: {entry['thread']})")
        colored_print(f"[WEBSOCKET] Connection attempts: {len(self.connection_attempts_details)}")
        if self.connection_attempts_details:
            for attempt in self.connection_attempts_details[-5:]:
                status = "✅" if attempt['success'] else "❌"
                colored_print(f"[WEBSOCKET]   {status} Attempt {attempt['attempt']}: {attempt['duration']:.1f}s")
        colored_print("[WEBSOCKET] === End Diagnostics ===")
...
def get_ticker_icon(ticker, size=32):
    ...
    ticker_candidates = get_icon_lookup_candidates(ticker)
    pixmap = None
    for candidate in ticker_candidates:
        local_path = os.path.join(images_dir, f"{candidate}.png")
    ...
    if pixmap is None or pixmap.isNull():
        for candidate in ticker_candidates:
            local_path = os.path.join(images_dir, f"{candidate}.png")
            url = f"https://raw.githubusercontent.com/krypdoh/stock-icons/refs/heads/main/ticker_icons/{candidate}.png"
            try:
    ...
    # Crypto fallback source: stock-icons/crypto_icons (e.g. ETH.png)
    if pixmap is None or pixmap.isNull():
        for candidate in get_crypto_icon_candidates(ticker):
            local_path = os.path.join(images_dir, f"{candidate}.png")
            url = f"https://raw.githubusercontent.com/krypdoh/stock-icons/main/crypto_icons/{candidate}.png"
            try:
                resp = get_requests_session().get(url, timeout=5)
                if resp.status_code == 200:
                    with open(local_path, "wb") as f:
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, tray_icon=None):
        ...
        scroll_area.setStyleSheet(
            "QScrollArea { background: #2b2b2b; border: 1px solid #3a3a3a; }"
            "QScrollArea QWidget#qt_scrollarea_viewport { background: #2b2b2b; }"
        )

        content = QtWidgets.QWidget()
        content.setFixedWidth(860)
        content.setAutoFillBackground(True)
        content_palette = content.palette()
        content_palette.setColor(QtGui.QPalette.Window, QtGui.QColor("#2b2b2b"))
        ...
        self.finnhub_api_key_edit.setMinimumWidth(wide_field_width)
        
        self.primary_key_toggle = QtWidgets.QPushButton("SHOW")
        self.primary_key_toggle.setFixedWidth(56)
        self.primary_key_toggle.setFixedHeight(input_height)
        ...
        self.finnhub_api_key_2_edit.setMinimumWidth(wide_field_width)
        
        self.secondary_key_toggle = QtWidgets.QPushButton("SHOW")
        self.secondary_key_toggle.setFixedWidth(56)
        self.secondary_key_toggle.setFixedHeight(input_height)
        ...
        ws_label = make_row_label("WS Info Refresh:")
        ws_label.setToolTip("Websocket Info Refresh: how often to update display with WebSocket prices\n0 = Disabled (perfectly smooth, no stutter)\n15-30 = Periodic updates (causes brief stutter)")
        self.ws_refresh_spin = QtWidgets.QSpinBox()
        self.ws_refresh_spin.setRange(0, 60)
        self.ws_refresh_spin.setValue(self.settings.get("websocket_visual_refresh_seconds", 2))
        self.ws_refresh_spin.setToolTip("Websocket Info Refresh\n0 = Smooth scrolling (no WS updates)\n15-30 = Periodic updates (brief stutter)")
        self.ws_refresh_spin.setFixedWidth(spin_col_width)
        self.ws_refresh_spin.setFixedHeight(input_height)

        ws_refresh_layout.addWidget(ws_label)
        ...
        api_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        layout.addWidget(api_group)
        ...
        self.second_ticker_stock_file_edit.setMinimumWidth(wide_field_width)
        ...
        stock_file_layout.setSpacing(2)
        stock_file_layout.addWidget(self.second_ticker_stock_file_edit)

        self.second_ticker_stock_browse_btn = QtWidgets.QPushButton("📁")
        self.second_ticker_stock_browse_btn.setFixedSize(22, 22)
        self.second_ticker_stock_browse_btn.setStyleSheet("QPushButton { font-size: 13px; padding: 0px; }")
        ...
        appearance_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        layout.addWidget(appearance_group)

        # === DUAL TICKER GROUP ===
        dual_ticker_group = QtWidgets.QGroupBox("⬛⬛ Dual Ticker")
        dual_ticker_layout = QtWidgets.QGridLayout(dual_ticker_group)
        dual_ticker_layout.setSpacing(4)
        dual_ticker_layout.setContentsMargins(8, 6, 8, 6)
        dual_ticker_layout.setColumnStretch(0, 1)
        dual_ticker_layout.setColumnStretch(1, 1)

        dual_ticker_layout.addWidget(enable_second_ticker_row, 0, 0)
        dual_ticker_layout.addWidget(second_ticker_speed_row, 0, 1)
        ...
        dual_ticker_layout.addWidget(second_ticker_stocks_row, 1, 0, 1, 2)

        dual_ticker_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        layout.addWidget(dual_ticker_group)
        ...
        self.transparency_spin.valueChanged.connect(lambda _: self.effects_preview.update())

        effects_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        ...
        sound_layout.setContentsMargins(8, 6, 8, 6)
        ...
        sound_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        sound_group.setMaximumWidth(170)
        misc_layout.addWidget(sound_group, 0, QtCore.Qt.AlignTop)
        ...
        net_layout.setContentsMargins(8, 6, 8, 6)
        ...
        self.proxy_edit.setMinimumWidth(wide_field_width)
        ...
        self.cert_file_edit.setMinimumWidth(wide_field_width)
        self.cert_file_btn = QtWidgets.QPushButton("📁")
        self.cert_file_btn.setFixedSize(22, 22)
        self.cert_file_btn.setStyleSheet("QPushButton { font-size: 13px; padding: 0px; }")
        ...
        net_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        ...
        startup_layout.setContentsMargins(8, 6, 8, 6)

        is_frozen = getattr(sys, 'frozen', False)
        self.run_at_startup_checkbox = QtWidgets.QCheckBox("Run at Windows Startup")
        self.run_at_startup_checkbox.setToolTip(
            "Automatically launch TCKR when Windows starts.\n"
            "Writes to HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run."
            if is_frozen else
            "Only available in the installed .exe build."
        )
        self.run_at_startup_checkbox.setEnabled(is_frozen)
        if is_frozen:
            # Read current registry state
            self.run_at_startup_checkbox.setChecked(self._read_run_at_startup())
        else:
            self.run_at_startup_checkbox.setChecked(False)
        startup_layout.addWidget(self.run_at_startup_checkbox)

        if not is_frozen:
            note = QtWidgets.QLabel("(exe build only)")
            note.setStyleSheet("color: #5a6070; font-size: 10px;")
            startup_layout.addWidget(note)

        startup_layout.addStretch(1)
        startup_group.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        startup_group.setMinimumWidth(160)
        misc_layout.addWidget(startup_group, 0, QtCore.Qt.AlignTop)
        ...
        btns.setStyleSheet("QDialogButtonBox { background: #1f1f1f; border: 1px solid #252525; border-radius: 6px; padding: 6px; }")
        btns.accepted.connect(self.accept)
        btns.rejected.connect(self.reject)
        btns.button(QtWidgets.QDialogButtonBox.Apply).clicked.connect(self.apply_settings)

        quit_btn = btns.addButton("Quit TCKR", QtWidgets.QDialogButtonBox.ActionRole)
        make_danger_button(quit_btn)
        quit_btn.clicked.connect(self.quit_tckr)
        ...
        QtCore.QTimer.singleShot(0, self._adjust_initial_height)
    ...
    def _adjust_initial_height(self):
        ...
        max_height = int(available.height() * 0.96)
        # QScrollArea.sizeHint() is tiny by default — measure the content widget directly
        content_h = self._content.sizeHint().height()
        # Overhead: outer margins top+bottom (16) + spacing (4) + button bar + padding (~52)
        target_h = min(content_h + 72, max_height)
    ...
    def quit_tckr(self):
        if self.tray_icon and hasattr(self.tray_icon, "safe_exit"):
            self.tray_icon.safe_exit()
            return
        app = QtWidgets.QApplication.instance()
        if app:
            app.quit()
    ...
    _STARTUP_REG_NAME = "TCKR"

    def _read_run_at_startup(self):
        """Return True if TCKR is registered to run at Windows startup."""
        if _winreg is None:
            return False
        try:
            key = _winreg.OpenKey(_winreg.HKEY_CURRENT_USER, self._STARTUP_REG_KEY, 0, _winreg.KEY_READ)
            _winreg.QueryValueEx(key, self._STARTUP_REG_NAME)
            _winreg.CloseKey(key)
            return True
        except (FileNotFoundError, OSError):
            return False

    def _apply_run_at_startup(self, enabled):
        """Add or remove TCKR from the Windows startup registry key."""
        if _winreg is None or not getattr(sys, 'frozen', False):
            return
        try:
            key = _winreg.OpenKey(
                _winreg.HKEY_CURRENT_USER, self._STARTUP_REG_KEY, 0,
                _winreg.KEY_SET_VALUE | _winreg.KEY_READ
            )
            if enabled:
                exe_path = sys.executable
                _winreg.SetValueEx(key, self._STARTUP_REG_NAME, 0, _winreg.REG_SZ, f'"{exe_path}" --minimized')
            else:
                try:
                    _winreg.DeleteValue(key, self._STARTUP_REG_NAME)
            ...
            QtWidgets.QMessageBox.warning(
                ...
                f"Could not update Windows startup registry:\n{e}"
            )
    ...
    def apply_settings(self):
        ...
        # Apply run-at-startup immediately (registry write, no restart needed)
        run_at_startup = self.run_at_startup_checkbox.isChecked() if getattr(sys, 'frozen', False) else False
        if run_at_startup != self._read_run_at_startup():
            self._apply_run_at_startup(run_at_startup)
        save_settings(s)
...
class _VSyncThread(QtCore.QThread):
    """
    Background thread that calls DwmFlush() once per vertical sync interval and
    emits `vsync` so the main thread can call update() at the exact VSync boundary.

    Why a separate thread:
    - DwmFlush() is a blocking call (~16.67 ms on 60 Hz). Calling it on the Qt
      event loop thread would freeze the UI the same way time.sleep() did.
    - Running it in a QThread lets the event loop stay free every frame while
      still achieving true VSync-locked scheduling via a queued signal.

    Fallback: if DWM is unavailable (non-Windows or DWM disabled), the thread
    sleeps ~16 ms per iteration — equivalent to the previous QTimer approach.
    """
    vsync = QtCore.pyqtSignal(float)  # carries perf_counter() value captured at VBlank
        ...
        while self._running:
            if self._dwm_available:
                try:
                    ...
                    # Blocks until DWM presents the next frame (true VSync lock)
                    ...
                    # After unlock it can stay throttled (e.g. 42 FPS = 23.8 ms)
                    # until something forces it to re-sync with the display.
                    # Strategy: if 10+ consecutive frames are >20 ms (i.e. <50 Hz)
                    # and we haven't reset in the last 30 s, pause 500 ms so DWM
                    # has time to reinitialise at the monitor's native rate.
                    if elapsed > 0.020:
                        _consecutive_slow += 1
                        if _consecutive_slow >= 10:
                            now = _time.perf_counter()
                            if now - _last_reset_time > 30.0:
                                _last_reset_time = now
                                _consecutive_slow = 0
                                print("[PERF] DwmFlush throttled after lock/unlock — "
                                      "pausing 500 ms for DWM to stabilise")
                                _time.sleep(0.5)
                    else:
                        _consecutive_slow = 0

                except Exception:
                    _time.sleep(0.016)  # Fallback if DwmFlush fails
                    vsync_time = _time.perf_counter()
            else:
                _time.sleep(0.016)  # Non-Windows / DWM-off fallback
                vsync_time = _time.perf_counter()
            if self._running:
...
class TickerGLWidget(QtWidgets.QWidget):  # Changed from QOpenGLWidget to QWidget
    def __init__(self, parent):
        ...
        colored_print(f"[PERF] Using DwmFlush VSync-locked rendering for perfectly smooth scrolling")

        # VSync-locked render scheduling via DwmFlush() in a background thread.
        # _VSyncThread blocks on DwmFlush() (wakes at exact VSync boundary) and
        # emits vsync → queued connection → main thread calls self.update().
        # This gives true 60.0 FPS lock with no beat-frequency judder, while
        # keeping the Qt event loop completely unblocked between frames.
        #
        # DUAL-TICKER FIX: Share one _VSyncThread across all TickerGLWidget instances.
        # With two widgets each running their own DwmFlush loop, both threads unblock
        # at the same VBlank and both queue update() → the main thread paints widget1
        # then widget2 in a single 16.67ms budget, effectively halving FPS to ~30.
        # By sharing one thread, a single VBlank drives both update() calls and both
        # paints happen inside the same frame window → true 60 FPS on dual tickers.
        app = QtWidgets.QApplication.instance()
        if not getattr(app, '_shared_vsync_thread', None) or \
                not app._shared_vsync_thread.isRunning():
            app._shared_vsync_thread = _VSyncThread()
            app._shared_vsync_thread.start()
            colored_print("[PERF] Created shared VSync thread for all ticker widgets")
        else:
            colored_print("[PERF] Reusing shared VSync thread (dual-ticker mode)")
        self._vsync_thread = app._shared_vsync_thread
        # FIFO queue of VBlank timestamps.  Each _on_vsync appends one entry;
        # paint_ticker pops the OLDEST (popleft) so it always uses the timestamp
        # from the VBlank that triggered IT — never a newer one that arrived while
        # this widget was waiting behind widget1 in the Qt event queue.
        self._vsync_queue = collections.deque(maxlen=8)
        self._vsync_thread.vsync.connect(self._on_vsync, QtCore.Qt.QueuedConnection)
        # Disconnect only this widget's slot when destroyed — do NOT stop the shared thread
        self.destroyed.connect(self._safe_vsync_disconnect)

    def _on_vsync(self, vsync_time: float):
        """Receive exact VSync timestamp from the shared thread, enqueue it, then request repaint.
        Using a FIFO deque (not a plain attribute) ensures paint_ticker always reads the timestamp
        from the VBlank that triggered this specific update() — even when widget2 paints after
        a later VBlank has already fired and enqueued its own timestamp."""
        self._vsync_queue.append(vsync_time)
        self.update()

    def _safe_vsync_disconnect(self):
        """Disconnect this widget's update slot from the shared VSync thread.
        Safe to call during widget destruction — does not stop the shared thread."""
        try:
            self._vsync_thread.vsync.disconnect(self._on_vsync)
        except Exception:
            pass  # Already disconnected or thread gone — harmless

    def paintEvent(self, event):
        # Track inter-frame delay for diagnostics (no sleep — _render_timer drives updates)
        if not hasattr(self, '_paint_call_time'):
            self._paint_call_time = 0
            self._paint_delay_warnings = 0

        call_time = self.ticker_window._time_module.perf_counter()
        ...
        # _vsync_thread emits vsync → self.update() via queued signal — no sleep here
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    def __init__(self, parent, ticker_window):
        ...
        self.stocks_action = menu.addAction("Manage Tickers")
        menu.addSeparator()
        self.fetch_now_action = menu.addAction("🔄️ Fetch Prices Now")
        ...
        self.fps_overlay_action = menu.addAction("Show FPS")
        self.fps_overlay_action.setCheckable(True)
        self.fps_overlay_action.setChecked(settings.get('show_fps_overlay', False))
        self.effects_action = menu.addAction("Enable Visual Effects (Bloom/Glow/Glass)")
    ...
    def show_websocket_status(self):
        ...
        def show_status_dialog(title, body, update_fn=None):
            dialog = QtWidgets.QDialog(self.primary_ticker)
            ...
            header = QtWidgets.QLabel("🌐 WebSocket Status")
            header.setStyleSheet("font-size: 14px; font-weight: 600; color: #19b5fe;")
            ...
            details.setStyleSheet(
                "QPlainTextEdit { background: #1f1f1f; border: 1px solid #3a3a3a; "
            ...
            if update_fn is not None:
                def _refresh():
                    try:
                        details.setPlainText(update_fn())
                    except Exception:
                        pass
                ...
                def _stop_timer(_result):
                    try:
                        timer.stop()
                    except Exception:
                        pass

                dialog.finished.connect(_stop_timer)
        ...
        def build_status_msg():
            ...
            finnhub_count = len([s for s in self.primary_ticker.stocks if not is_yahoo_symbol(s)])
        ...
        show_status_dialog("WebSocket Status", build_status_msg(), update_fn=build_status_msg)
    ...
    def show_settings(self):
        ...
        dlg = SettingsDialog(None, tray_icon=self)
        dlg.setModal(False)
        ...
        def _save_and_clear_settings_dialog(_result):
            # Remember position for next open this session (not persisted to disk)
            self._settings_dialog_last_pos = dlg.pos()
            self._settings_dialog = None

        def _clamp_dialog_to_screen(dialog):
            """Ensure dialog is fully visible on some screen after a position restore."""
            pos = dialog.pos()
            size = dialog.size()
            ...
            x = max(sg.left(), min(pos.x(), sg.right() - size.width()))
            y = max(sg.top(), min(pos.y(), sg.bottom() - size.height()))
            if x != pos.x() or y != pos.y():
                dialog.move(x, y)

        def _position_settings_dialog():
            # Subsequent opens this session: restore last closed position
            last_pos = getattr(self, '_settings_dialog_last_pos', None)
            if last_pos is not None:
                dlg.move(last_pos)
                _clamp_dialog_to_screen(dlg)
                return

            # First open this session: place below the ticker on its screen
            ticker = getattr(self, 'primary_ticker', None)
            screen = None
            ticker_bottom_y = 0

            if ticker is not None:
                fg = ticker.frameGeometry()
                ticker_bottom_y = fg.bottom() + 1
                screen = QtWidgets.QApplication.screenAt(
                    QtCore.QPoint(fg.left() + fg.width() // 2, fg.top())
                )
            ...
            sg = screen.availableGeometry()
            x = sg.left() + (sg.width() - dlg.width()) // 2
            y = ticker_bottom_y
            x = max(sg.left(), min(x, sg.right() - dlg.width()))
            y = max(sg.top(), min(y, sg.bottom() - dlg.height()))
            dlg.move(x, y)

        dlg.accepted.connect(_on_settings_accepted)
        dlg.finished.connect(_save_and_clear_settings_dialog)
        self._settings_dialog = dlg
        dlg.show()
        QtCore.QTimer.singleShot(0, _position_settings_dialog)
        dlg.raise_()
        dlg.activateWindow()
    ...
    def _handle_second_ticker_restart(self, settings):
        ...
        if current_window_count > 1:
            if not enable_second_ticker:
                ...
                # Second ticker remains enabled — refresh its stock list in case the
                # custom file path changed without a full window rebuild.
                stock_file = settings.get("second_ticker_stock_file", "").strip()
                second_win = self.ticker_windows[1]
                if stock_file and os.path.exists(stock_file):
                    try:
                        with open(stock_file, "r") as f:
                            second_stocks = json.load(f)
                        second_win.stocks = [s[0] for s in second_stocks]
                        second_win._custom_stocks = list(second_win.stocks)
                        print(f"[RESTART] Refreshed second ticker with {len(second_win.stocks)} stocks from custom file")
                        second_win.build_ticker_text(reset_scroll=False, use_incremental_rebuild=True)
                    except Exception as e:
                        print(f"[RESTART] Failed to refresh custom stock file: {e}")
                else:
                    # No custom file — use primary stocks and clear the custom marker
                    second_win.stocks = self.primary_ticker.stocks.copy()
                    second_win._custom_stocks = None
                    print(f"[RESTART] Refreshed second ticker with main stocks ({len(second_win.stocks)} stocks)")
                    second_win.build_ticker_text(reset_scroll=False, use_incremental_rebuild=True)
            return
            ...
            try:
                ...
                if stock_file and os.path.exists(stock_file):
                    try:
                        ...
                        # Persist so price-distribution loops don't overwrite with main stocks
                        second_ticker_window._custom_stocks = list(second_ticker_window.stocks)
    ...
    def show_manage_stocks(self):
        ...
        def _on_manage_stocks_accepted():
            ...
            # Immediately fetch newly added symbols so they don't remain N/A until next cycle
            import time as time_module
            previous_symbols = list(getattr(self.primary_ticker, 'stocks', []))
            previous_set = set(previous_symbols)

            # Refresh cached settings so changes from Manage Tickers (e.g. show_market_status) take effect
            fresh_settings = get_settings()
            for ticker in self.ticker_windows:
                ticker._cached_settings = fresh_settings

            self.primary_ticker.stocks = [s[0] for s in load_stocks()]
            current_symbols = list(self.primary_ticker.stocks)
            current_set = set(current_symbols)
            added_symbols = [s for s in current_symbols if s not in previous_set]
            removed_symbols = [s for s in previous_symbols if s not in current_set]

            # Drop removed symbols from cached prices so stale entries don't linger
            for removed_symbol in removed_symbols:
                self.primary_ticker.prices.pop(removed_symbol, None)
            ...
            for ticker in self.ticker_windows:
                if ticker != self.primary_ticker:
                    ...
                    for removed_symbol in removed_symbols:
                        ticker.prices.pop(removed_symbol, None)
                    ticker.build_ticker_text(reset_scroll=True)

            if not added_symbols:
                print("[MANAGE STOCKS] Stock list updated for all tickers - no newly added symbols to fetch")
                return

            print(f"[MANAGE STOCKS] Stock list updated for all tickers - fetching {len(added_symbols)} newly added symbols now")

            # Subscribe newly added Finnhub symbols to websocket immediately
            try:
                finnhub_added = [s for s in added_symbols if not is_yahoo_symbol(s)]
                if finnhub_added and getattr(self.primary_ticker, 'websocket_client', None):
                    self.primary_ticker.websocket_client.subscribe_symbols(finnhub_added)
            except Exception as e:
                print(f"[MANAGE STOCKS] Websocket subscribe for new symbols failed: {e}")

            def _apply_added_symbol_prices(prices):
                try:
                    targets = self.ticker_windows if hasattr(self, 'ticker_windows') else [self.primary_ticker]
                    updated_total = 0
                    unresolved_symbols = []
                    for ticker in targets:
                        updated = []
                        for symbol in added_symbols:
                            if not hasattr(ticker, 'stocks') or symbol not in ticker.stocks:
                                continue
                            fetched_price, fetched_prev_close = prices.get(symbol, (None, None))
                            old_price, old_prev_close = ticker.prices.get(symbol, (None, None))

                            if fetched_price is None:
                                fetched_price = old_price
                            if fetched_prev_close is None and old_prev_close is not None:
                                fetched_prev_close = old_prev_close

                            if fetched_price is None and fetched_prev_close is None:
                                if symbol not in unresolved_symbols:
                                    unresolved_symbols.append(symbol)
                                continue

                            ticker.prices[symbol] = (fetched_price, fetched_prev_close)
                    ...
                    print(f"[MANAGE STOCKS] Immediate fetch applied for {updated_total} symbol entries")
                    if unresolved_symbols:
                        preview = ", ".join(unresolved_symbols[:12])
                        suffix = "..." if len(unresolved_symbols) > 12 else ""
                        print(f"[MANAGE STOCKS] Symbols still unresolved after immediate fetch ({len(unresolved_symbols)}): {preview}{suffix}")
                except Exception as e:
                    print(f"[MANAGE STOCKS] Failed applying immediate prices: {e}")
            ...
            try:
                ...
                prices, _had_429 = fetch_all_stock_prices_with_429(added_symbols, api_key, api_key_2, force=True)
            except Exception as e:
                print(f"[MANAGE STOCKS] Immediate fetch failed: {e}")
            _apply_added_symbol_prices(prices)
    ...
    def show_about(self):
        ...
        # Version
        version = QtWidgets.QLabel("Version 1.1.5")
        ...
        links_label = QtWidgets.QLabel(
            '<a href="https://krypdoh.github.io/TCKR/" style="color: #00b3ff;">TCKR Website</a>'
        )
...
class TickerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, is_secondary=False):
        ...
        self._pixmap_rebuild_pending = False  # Coalescing flag for deferred pixmap rebuild
        ...
        if self.websocket_enabled:
            ...
            if api_key:
                ...
                refresh_interval = settings.get('websocket_visual_refresh_seconds', 2)  # 2s default
    ...
    def refresh_websocket_visuals(self):
        ...
        try:
            # Drain the entire price buffer in one call — dict writes are essentially
            # free; only symbols where the displayed text changed are returned for
            # pixmap rebuilds, keeping the incremental rebuild queue short.
            updates_sent, updated_symbols = self.websocket_client.process_buffered_updates()
            ...
            if updates_sent > 0 and not self.loading and self.stocks:
                ...
                if self._websocket_refresh_count % 10 == 0:
                    colored_print(f"[WEBSOCKET] 🎨 Visual refresh #{self._websocket_refresh_count}: {updates_sent} prices updated, {len(updated_symbols)} displays changed")

            # Buffer is fully drained each call — no slice continuation needed
            ...
            if self.websocket_client and self.websocket_client.connected:
                ...
                if missing_prev_close:
                    ...
                    if now - last_refresh > 45:
                        ...
                        self._refresh_missing_prev_close(missing_prev_close[:20])
        finally:
            self._websocket_busy = False
    ...
    def check_websocket_cost_startup(self):
        ...
        finnhub_stocks = len([s for s in self.stocks if not is_yahoo_symbol(s)])
        ...
        if market_open and self.websocket_client and not self.websocket_client.connected:
            ...
            if self.websocket_client.connect():
                ...
                finnhub_symbols = [s for s in self.stocks if not is_yahoo_symbol(s)]
                if finnhub_symbols:
    ...
    def get_smart_update_interval(self):
        ...
        import time
        current_time = time.time()
        has_crypto_symbols = any(is_crypto_symbol(symbol) for symbol in getattr(self, 'stocks', []))
        ...
        if current_time - self._last_market_check > 300:
            try:
                ...
                if market_open:
                    ...
                    interval = self.update_interval if has_crypto_symbols else max(self.update_interval * 3, 900000)  # Keep normal cadence when crypto is selected
                    status_reason = "CLOSED" if not is_holiday and now_et.weekday() < 5 else "CLOSED"
                    if has_crypto_symbols:
                        colored_print(f"[MARKET] 🌙 Market {status_reason} - {', '.join(status_parts)} (normal updates for crypto)")
                    else:
                ...
                if now.weekday() < 5 and 9.5 <= now.hour + now.minute/60 <= 16:
                    ...
                    interval = self.update_interval if has_crypto_symbols else max(self.update_interval * 3, 900000)
                    if has_crypto_symbols:
                        colored_print(f"[MARKET] 🌙 Market CLOSED - {now.strftime('%H:%M')} local time (normal updates for crypto)")
                    else:
        ...
        if not self._market_is_open and has_crypto_symbols:
            return self.update_interval
    ...
    def ensure_top_position(self):
        ...
        # Primary ticker - force to the negotiated appbar top (may be below a foreign app)
        expected_top = getattr(self, '_appbar_top_logical', rect.top())
        self.move(rect.left(), expected_top)
        self.resize(rect.width(), self.ticker_height)

        # Use aggressive positioning on Windows
        if sys.platform == "win32":
            # Set TOPMOST z-order using SWP_NOMOVE|SWP_NOSIZE so we don't pass
            # logical coordinates to a Win32 API that expects physical pixels.
            # Qt's move()/resize() above already handled the physical positioning.
            HWND_TOPMOST  = -1
            SWP_NOMOVE    = 0x0002
            SWP_NOSIZE    = 0x0001
            SWP_NOACTIVATE = 0x0010
            ctypes.windll.user32.SetWindowPos(
                ...
                SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE)
            final_pos = self.pos()
            print(f"[ENSURE TOP] Positioned at y={final_pos.y()} (target={expected_top})")
    ...
    def coordinate_with_other_tickers(self):
        ...
        try:
            ...
            if ticker_windows:
                ...
                # Use the screen's current available-geometry top as the base offset.
                # At this point the primary ticker has NOT yet registered its AppBar
                # (it fires 550ms later), so availableGeometry().top() reflects the
                # foreign-app reservation only (e.g. 72 logical px).
                # We add one ticker_height per slot above us.
                try:
                    avail_top_logical = screen.availableGeometry().top()
                except Exception:
                    avail_top_logical = screen_rect.top()

                new_top_logical = avail_top_logical + self.ticker_height
                print(f"[COORDINATE] avail_top={avail_top_logical}, ticker_height={self.ticker_height}, positioning at logical y={new_top_logical}")
    ...
    def check_and_fix_position(self):
        ...
        # Use the negotiated AppBar top (logical pixels) — may be > rect.top() when a foreign
        # app occupies the screen top edge.
        expected_top = getattr(self, '_appbar_top_logical', rect.top())
        
        # If window has drifted from the expected position, fix it
        if current_pos.y() > expected_top + 5:  # Allow 5px tolerance
            ...
            # Restore TOPMOST z-order without changing position (avoids logical/physical mismatch)
            HWND_TOPMOST = -1
    ...
    def setup_appbar_and_position(self):
        ...
        if hasattr(self, 'is_secondary_ticker') and self.is_secondary_ticker:
            ...
            # Position at the initial estimate immediately so the window is visible
            if hasattr(self, 'target_position_y'):
                self.setGeometry(rect.left(), self.target_position_y, rect.width(), self.ticker_height)
                self._appbar_top_logical = self.target_position_y
                if DEBUG_APPBAR: print(f"[APPBAR] Secondary ticker initially at y={self.target_position_y}")

            # Schedule a delayed re-check at 750ms: by then the primary ticker will have
            # registered its AppBar (~550ms) and been repositioned by Qt. Find the primary
            # ticker's actual logical position and snap this window right below it.
            def _reposition_after_primary():
                try:
                    primary_bottom = None
                    if hasattr(self, 'tray_icon') and hasattr(self.tray_icon, 'ticker_windows'):
                        for w in self.tray_icon.ticker_windows:
                            if w is not self and not getattr(w, 'is_secondary_ticker', False):
                                primary_bottom = w.pos().y() + w.ticker_height
                                break
                    if primary_bottom is not None and primary_bottom != self.pos().y():
                        print(f"[SECONDARY] Delayed reposition: y={self.pos().y()} -> {primary_bottom}")
                        self.setGeometry(rect.left(), primary_bottom, rect.width(), self.ticker_height)
                        self.target_position_y = primary_bottom
                        self._appbar_top_logical = primary_bottom
                        _TOPMOST = -1; _NOMOVE = 0x0002; _NOSIZE = 0x0001; _NOACT = 0x0010
                        ctypes.windll.user32.SetWindowPos(int(self.winId()), _TOPMOST, 0, 0, 0, 0,
                                                          _NOMOVE | _NOSIZE | _NOACT)
                    else:
                        print(f"[SECONDARY] Delayed reposition: already correct at y={self.pos().y()}")
                except Exception as e:
                    print(f"[SECONDARY] Delayed reposition error: {e}")

            QtCore.QTimer.singleShot(750, _reposition_after_primary)
        ...
        # Determine DPI scale factor for this screen (logical -> physical)
        device_scale = 1.0
        try:
        ...
        # Capture work-area top in physical pixels for later use in closures
        work_area_top_phys = int(work_area.top)

        foreign_app_at_top = False
        if existing_reservation_phys > 0 and not has_other_tickers:
            # Determine whether the reservation belongs to a live foreign application
            # or is stale from a previous TCKR run. Enumerate visible top-level windows
            # that overlap the reserved band. All Win32 coords are physical pixels.
            try:
                phys_screen_left  = int(round(rect.left() * device_scale))
                phys_screen_right = int(round((rect.left() + rect.width()) * device_scale))
                found_foreign = [False]
                own_hwnd = int(self.winId())

                def _is_foreign_at_top(hwnd, lParam):
                    try:
                        if hwnd == own_hwnd or not user32.IsWindowVisible(hwnd):
                            return True
                        wr = wintypes.RECT()
                        user32.GetWindowRect(hwnd, ctypes.byref(wr))
                        overlaps_top = (wr.top <= phys_screen_top + 5) and (wr.bottom > phys_screen_top + 5)
                        overlaps_x   = (wr.right > phys_screen_left) and (wr.left < phys_screen_right)
                        if overlaps_top and overlaps_x:
                            found_foreign[0] = True
                    except Exception:
                        pass
                    return True

                _ForeignProc = ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int))
                user32.EnumWindows(_ForeignProc(_is_foreign_at_top), 0)
                foreign_app_at_top = found_foreign[0]
            except Exception:
                pass

            if not foreign_app_at_top:
                if DEBUG_APPBAR: print(f"[APPBAR] Stale work-area reservation ({existing_reservation_phys}px phys) with no live tickers - clearing it")
                user32.SystemParametersInfoW(47, 0, None, 0x0002)  # SPI_SETWORKAREA, let Windows recalculate
                existing_reservation_phys = 0
                existing_reservation_logical = 0
            else:
                if DEBUG_APPBAR: print(f"[APPBAR] Foreign app occupies top {existing_reservation_phys}px phys ({existing_reservation_logical}px logical) - will register TCKR below it")

        if existing_reservation_phys > 0 and not foreign_app_at_top:
            if DEBUG_APPBAR: print(f"[APPBAR] Space already reserved at top ({existing_reservation_phys}px phys / {existing_reservation_logical}px logical)")
            
            # Get our actual physical window height
            hwnd = int(self.winId())
            actual_window_rect = wintypes.RECT()
            user32.GetWindowRect(hwnd, ctypes.byref(actual_window_rect))
            actual_window_height = actual_window_rect.bottom - actual_window_rect.top
            ...
            # How many ticker slots are already reserved?
            num_existing_slots = existing_reservation_phys // actual_window_height if actual_window_height else 0
            if DEBUG_APPBAR: print(f"[APPBAR] Existing slots: {num_existing_slots}")
            
            our_slot = num_existing_slots
            # target in logical pixels: each slot is self.ticker_height logical px
            target_y_logical = rect.top() + (our_slot * self.ticker_height)
            
            if DEBUG_APPBAR: print(f"[APPBAR] We are ticker #{our_slot + 1}, positioning at logical y={target_y_logical}")
            
            self.setGeometry(rect.left(), target_y_logical, rect.width(), self.ticker_height)
            self.is_secondary_ticker = True
            self.target_position_y = target_y_logical
            ...
            def check_position():
                pos = self.pos()
                if pos.y() != target_y_logical:
                    if DEBUG_APPBAR: print(f"[APPBAR] Position drifted to y={pos.y()}, fixing to y={target_y_logical}")
                    self.setGeometry(rect.left(), target_y_logical, rect.width(), self.ticker_height)
        ...
        def after_removal():
            ...
            def setup_new_appbar():
                ...
                # device_scale captured in outer scope above

                # When a foreign app occupies the top, TCKR starts immediately below it.
                # start_y_phys is the physical-pixel top edge for the AppBar rect.
                # initial_y_logical is the logical-pixel y for Qt's setGeometry.
                if foreign_app_at_top:
                    start_y_phys   = work_area_top_phys          # physical
                    initial_y_logical = int(work_area_top_phys / device_scale)  # logical
                else:
                    start_y_phys   = None
                    initial_y_logical = rect.top()               # logical

                print(f"[SETUP] PRIMARY ticker - moving to top: x={rect.left()}, y={initial_y_logical} (logical), device_scale={device_scale}")
                self.setGeometry(rect.left(), initial_y_logical, rect.width(), self.ticker_height)
                
                pos = self.pos()
                print(f"[SETUP] Position after move: x={pos.x()}, y={pos.y()}")
                
                # Combined physical height for ALL managed tickers on this screen
                combined_logical = 0
                if hasattr(self, 'tray_icon') and hasattr(self.tray_icon, 'ticker_windows'):
                    for w in self.tray_icon.ticker_windows:
                        try:
                ...
                desired_physical_height = int(round(combined_logical * device_scale))
                ...
                if DEBUG_APPBAR: print(f"[APPBAR] Registering PRIMARY ticker as appbar: desired_physical_height={desired_physical_height}, screen={rect}, start_y_phys={start_y_phys}")
                # set_appbar now takes physical height + logical rect + device_scale, returns logical top
                actual_top_logical = set_appbar(int(self.winId()), desired_physical_height, rect,
                                                start_y=start_y_phys, device_scale=device_scale)
                ...
                if DEBUG_APPBAR: print(f"[APPBAR] set_appbar returned actual_top_logical={actual_top_logical}")
                # actual_top_logical is already in Qt logical pixels
                self.setGeometry(rect.left(), int(actual_top_logical), rect.width(), self.ticker_height)
                # Store negotiated top so ensure_top_position doesn't override it
                self._appbar_top_logical = int(actual_top_logical)
                pos = self.pos()
                if DEBUG_APPBAR: print(f"[APPBAR] Position after setGeometry: x={pos.x()}, y={pos.y()}")
                if DEBUG_POSITIONING: print(f"[POSITIONING] Screen top: {rect.top()}, AppBar negotiated logical: {actual_top_logical}, Qt pos: {pos.y()}")
    ...
    def move_to_selected_screen(self):
        ...
        if sys.platform == "win32":
            ...
            def after_cleanup():
                ...
                actual_top = set_appbar(int(self.winId()), desired_physical_height, rect, device_scale=dpr)

                # Position at the negotiated AppBar top (respects other AppBars already present)
                # set_appbar now returns logical pixels
                self.move(rect.left(), int(actual_top))
                self.resize(rect.width(), self.ticker_height)
                # Keep ensure_top_position in sync with the negotiated position
                self._appbar_top_logical = int(actual_top)
                print(f"[SCREEN MOVE] Screen top: {rect.top()}, AppBar negotiated logical: {actual_top}")
    ...
    def update_prices_full(self):
        ...
        # Get all stocks/indices — also collect custom stocks from any secondary tickers
        # so that symbols unique to a custom file are fetched in the initial load rather
        # than waiting for the first coordinated-fetch timer (up to 5 minutes away).
        all_stocks = set(s[0] for s in load_stocks())
        if hasattr(self, 'tray_icon') and self.tray_icon and hasattr(self.tray_icon, 'ticker_windows'):
            for _win in self.tray_icon.ticker_windows:
                if _win is not self and getattr(_win, '_custom_stocks', None):
                    all_stocks.update(_win._custom_stocks)
        all_stocks = list(all_stocks)
        yahoo_tickers = [t for t in all_stocks if is_yahoo_symbol(t)]
        finnhub_tickers = [t for t in all_stocks if not is_yahoo_symbol(t)]
        ...
        if not api_key and finnhub_tickers:
            ...
            colored_print(f"[TCKR] No API key found - fetching only {len(yahoo_tickers)} Yahoo Finance symbols")
            ...
            if yahoo_tickers:
                ...
                colored_print("[TCKR] No API key and no Yahoo symbols selected - nothing to display")
                self.loading = False
                self.gl_widget.update()
            return
        ...
        if (hasattr(self, 'tray_icon') and self.tray_icon and
            ...
            for ticker in self.tray_icon.ticker_windows:
                if ticker != self:  # Don't update self again
                    # Respect secondary ticker's custom file stocks; only fall back to
                    # main stocks list when no custom file was configured.
                    if getattr(ticker, '_custom_stocks', None) is None:
                    ...
                    ticker.build_ticker_text(reset_scroll=False, use_incremental_rebuild=True)
    ...
    def _handle_coordinated_prices(self, prices, had_429, now):
        ...
        # Distribute prices to all tickers; note that secondary tickers may have
        # a custom stock file — only update their stocks list when no custom file is set.
        # Read the main stocks list once (disk I/O) for tickers without a custom file.
        _stocks_list = [s[0] for s in load_stocks()]
        for ticker in self.tray_icon.ticker_windows:
            # Ensure each ticker has the normalized stocks list before rebuilding
            if getattr(ticker, '_custom_stocks', None) is None:
                ticker.stocks = list(_stocks_list)
            # Start from existing prices so tickers NOT in this fetch (e.g. Finnhub tickers
            # during WebSocket-only Yahoo polling) are preserved rather than wiped.
            merged_prices = ticker.prices.copy()
            for tkr, (price, prev_close) in prices.items():
                old_price, old_prev_close = merged_prices.get(tkr, (None, None))
                if price is None and old_price is not None:
                    price = old_price
                if prev_close is None and old_prev_close is not None:
                    prev_close = old_prev_close
            ...
            # Update the ticker display — keep scroll position intact so the
            # ticker continues seamlessly; spread the pixmap rebuild over frames
            # to avoid blocking the render loop (no scroll reset, incremental rebuild)
            ticker.build_ticker_text(reset_scroll=False, use_incremental_rebuild=True)
    ...
    def get_glow_effect(self, symbol, change_percent):
        ...
        if symbol in self.pulse_effects:
            ...
            # Keep glow noticeable but less intense for readability
            intensity = min(abs(change_percent), 10.0)
            glow_alpha = int(20 + max(0.0, intensity - 5.0) * 3.0)
    ...
    def update_prices_inplace(self, force=False):
        ...
        if hasattr(self, 'tray_icon') and self.tray_icon and self == self.tray_icon.primary_ticker:
            ...
            try:
                ...
                colored_print(f"[COORDINATED FETCH] Unexpected error in price fetching: {type(_coord_err).__name__}: {_coord_err}")
            return
    ...
    def _coordinate_price_fetching(self, force, now):
        ...
        # Only log on first run or when forced — not on every recurring timer tick
        _verbose = force or not hasattr(self, '_coord_fetch_count')
        if not hasattr(self, '_coord_fetch_count'):
            self._coord_fetch_count = 0
        self._coord_fetch_count += 1
        if _verbose:
            print(f"[COORDINATED FETCH] Fetching {len(combined_stocks)} unique stocks for {len(self.tray_icon.ticker_windows)} tickers")

        yahoo_tickers = [t for t in combined_stocks if is_yahoo_symbol(t)]
        finnhub_tickers = [t for t in combined_stocks if not is_yahoo_symbol(t)]
        ...
        if self.websocket_client and market_is_open and not force:
            ...
            if not self.websocket_client.connected:
                ...
                if self.websocket_client.connect():
                    ...
                    finnhub_symbols = [s for s in combined_stocks if not is_yahoo_symbol(s)]
                    if finnhub_symbols:
            ...
            if real_time_available:
                # Log status periodically off the main thread so stdout flushing
                ...
                if hasattr(self, '_last_websocket_status_log'):
                    if time_module.time() - self._last_websocket_status_log > 300:  # Every 5 minutes
                        ...
                        _ws = self.websocket_client
                        import threading as _thr
                        _thr.Thread(target=_ws.log_status_summary, daemon=True).start()
                else:
                    self._last_websocket_status_log = time_module.time()

                # Check cost warning for free tier
                finnhub_count = len([s for s in combined_stocks if not is_yahoo_symbol(s)])
                if finnhub_count > 50:
                ...
                # Keep polling Yahoo symbols on normal schedule (including always-open crypto)
                if yahoo_tickers:
                    tickers_to_fetch = yahoo_tickers
                else:
                    return
            else:
                tickers_to_fetch = combined_stocks
        else:
            tickers_to_fetch = combined_stocks
        ...
        if not force and hasattr(TickerWindow, 'backoff_until') and now < TickerWindow.backoff_until:
            if tickers_to_fetch and all(is_yahoo_symbol(t) for t in tickers_to_fetch):
                colored_print("[UPDATE] In Finnhub backoff, continuing Yahoo-only polling")
            else:
        ...
        if self.websocket_client and not market_is_open and self.websocket_client.connected:
            ...
            _ws = self.websocket_client
            import threading as _thr
            _thr.Thread(target=lambda: (_ws.log_status_summary(), _ws.disconnect()), daemon=True).start()

        # Use the already-loaded _cached_settings to avoid a disk read on the main thread.
        # Only fall back to the full ensure_finnhub_api_key dialog path when no key is cached.
        _cs = self._cached_settings if hasattr(self, '_cached_settings') else get_settings()
        api_key = _cs.get("finnhub_api_key", "").strip() or None
        if not api_key:
            api_key = ensure_finnhub_api_key(self)  # Shows dialog only when key is truly absent

        if not api_key:
            # No API key - only fetch Yahoo Finance symbols (no key needed)
            if not yahoo_tickers:
                # print("[BACKOFF DEBUG] No API key and no Yahoo symbols, aborting fetch.")  # Commented for less verbose output
                return
            # Only fetch Yahoo symbols
            tickers_to_fetch = yahoo_tickers
        elif not tickers_to_fetch:
            tickers_to_fetch = combined_stocks

        # Get second API key if configured (reuse same cached settings dict)
        api_key_2 = _cs.get("finnhub_api_key_2", "").strip() or None
        ...
        # Wrapped in try/except: Python 3.14 threading internals (Event/Condition/Lock chain)
        # can transiently raise TypeError after long uptime. Skip the cycle and let the
        # next timer tick retry automatically rather than crashing the application.
        import threading
        try:
            fetch_thread = threading.Thread(target=fetch_and_handle, daemon=True)
            fetch_thread.start()
        except (TypeError, RuntimeError, OSError) as _thread_err:
            colored_print(f"[COORDINATED FETCH] Thread creation failed ({type(_thread_err).__name__}: {_thread_err}) — skipping cycle, will retry on next tick")
    ...
    def _process_price_update_deferred(self):
        ...
        if USE_OPT and len(new_prices) > 0:  # Use Numba optimization for all non-empty batches
            try:
                # Preallocate arrays up-front to avoid list→np.array conversion overhead
                n = len(new_prices)
                tickers = []
                current_array = np.empty((n, 2), dtype=np.float32)
                old_array = np.empty((n, 2), dtype=np.float32)
                count = 0
                ...
                for tkr, (price, prev_close) in new_prices.items():
                    ...
                    if price is not None and prev_close is not None and price > 0 and prev_close > 0:
                        tickers.append(tkr)
                        current_array[count, 0] = price
                        current_array[count, 1] = prev_close
                        old_array[count, 0] = old_price if old_price is not None else price
                        old_array[count, 1] = prev_close
                        count += 1
                
                if count > 0:
                    # Batch calculate all price changes (np.empty gives C-contiguous arrays)
                    results = opt.batch_calculate_price_changes_optimized(current_array[:count], old_array[:count])
                    
                    # Process results
                    for i, tkr in enumerate(tickers):
                        price = float(current_array[i, 0])
                        prev_close = float(current_array[i, 1])
                        old_price = float(old_array[i, 0])
        ...
        # Original processing (fallback only - Numba path handles all non-empty batches)
        if not USE_OPT or len(new_prices) <= 0 or 'USE_OPT_FALLBACK' in locals():
            for tkr, (price, prev_close) in new_prices.items():
                old_price = self.prices.get(tkr, (None, None))[0]
                old_prev_close = self.prices.get(tkr, (None, None))[1]
        ...
        if USE_OPT and len(new_prices) > 0:  # Use parallel Numba for all non-empty price sets
            try:
                ...
                if valid_tickers:
                    # Use parallel glow detection (ascontiguousarray ensures optimal Numba memory layout)
                    prices_array = np.ascontiguousarray(prices_list, dtype=np.float32)
                    prev_closes_array = np.ascontiguousarray(prev_closes_list, dtype=np.float32)
        ...
        # Original sequential processing (fallback only)
        if not USE_OPT or len(new_prices) <= 0 or 'USE_PARALLEL_FALLBACK' in locals():
            for tkr, (price, prev_close) in new_prices.items():
                if price is not None and prev_close is not None and prev_close != 0:
                    if USE_OPT:
                        change_percent = opt.calculate_abs_change_percent(price, prev_close)
    ...
    def _rebuild_pixmaps_deferred(self):
        """Schedule a pixmap rebuild to run after all pending paint events complete.

        Uses a coalescing flag so that rapid back-to-back calls (e.g. multiple
        symbol price updates arriving in the same API batch) only trigger ONE
        rebuild instead of N, and defers that rebuild until the event loop is
        idle so it never stalls paintEvent mid-frame.
        """
        if getattr(self, '_pixmap_rebuild_pending', False):
            return  # A rebuild is already queued — no need to queue another
        self._pixmap_rebuild_pending = True
        QtCore.QTimer.singleShot(0, self._do_deferred_rebuild)

    def _do_deferred_rebuild(self):
        """Execute the coalesced pixmap rebuild (called from event loop, never mid-frame)."""
        self._pixmap_rebuild_pending = False
        try:
            self.build_ticker_pixmaps()
            if hasattr(self, 'gl_widget') and self.gl_widget:
                self.gl_widget.update()
        except Exception as e:
            colored_print(f"[PIXMAP] Error in deferred rebuild: {e}")
    ...
    def update_prices(self):
        ...
        self.stocks = [s[0] for s in load_stocks()]
        yahoo_tickers = [t for t in self.stocks if is_yahoo_symbol(t)]
        
        if not api_key:
            # No API key - only fetch Yahoo Finance symbols
            if yahoo_tickers:
                print(f"[UPDATE] No API key - fetching only {len(yahoo_tickers)} Yahoo Finance symbols")
                self.prices = fetch_all_stock_prices(yahoo_tickers, "", None)
            else:
                print("[UPDATE] No API key and no Yahoo symbols - nothing to update")
                return
        else:
            # Get second API key if configured
            api_key_2 = get_settings().get("finnhub_api_key_2", "").strip() or None
    ...
    def update_market_status(self):
        ...
        current_market_open = is_market_open()

        # Log websocket status periodically (off the hot path — use a background thread
        # so stdout flushing doesn't stall the render loop)
        if self.websocket_client:
            if current_market_open and self.websocket_client.connected:
                status = self.websocket_client.get_debug_status()
                if status['price_updates_processed'] > 0:
                    def _log_ws_status(s=status):
                        colored_print(f"[WEBSOCKET] 📊 Hourly status: {s['price_updates_processed']} updates, "
                                      f"{s['messages_received']} msgs, {s['connection_duration']:.1f}s connected")
                    import threading as _threading
                    _threading.Thread(target=_log_ws_status, daemon=True).start()

        # Only rebuild pixmaps when the market open/closed state actually transitions
        # (happens once at 9:30 AM and once at 4:00 PM).  Rebuilding every 60 s
        # unconditionally was blocking the render loop for ~tens of ms each time.
        prev_state = getattr(self, '_last_known_market_open', None)
        self._market_is_open = current_market_open
        if current_market_open != prev_state:
            self._last_known_market_open = current_market_open
            # Market just opened or closed — full rebuild is needed so the market-status
            # pixmap (index 0) is regenerated with the new state.  This happens at most
            # twice per day so a synchronous rebuild here is acceptable.
            self.build_ticker_text(reset_scroll=False)
    
    def build_ticker_text(self, reset_scroll=False, use_incremental_rebuild=False):
        items = []
        ...
        for tkr in self.stocks:
            display_name = ticker_display_name(tkr)
        ...
        if use_incremental_rebuild:
            # Spread the pixmap rebuild across multiple frames so the render loop
            # is never blocked for a long time (avoids scroll stutter on price refresh)
            self.queue_incremental_pixmap_updates(list(self.stocks))
        else:
            self.build_ticker_pixmaps()
    ...
    def build_ticker_pixmaps(self):
        ...
        # Optional market status icon (user-provided files for PyInstaller bundle)
        market_icon = None
        market_icon_name = "neon_check.png" if status_text == "Open" else "neon_cross.png"
        market_icon_path = resource_path(market_icon_name)

        if os.path.exists(market_icon_path):
            if not hasattr(self, '_market_status_icon_cache'):
                self._market_status_icon_cache = {}
            market_icon_size = max(14, int(self.ticker_height * 0.52))
            icon_cache_key = (market_icon_path, market_icon_size)
            market_icon = self._market_status_icon_cache.get(icon_cache_key)
            if market_icon is None:
                loaded_icon = QtGui.QPixmap(market_icon_path)
                if not loaded_icon.isNull():
                    market_icon = loaded_icon.scaled(
                        market_icon_size,
                        market_icon_size,
                        QtCore.Qt.KeepAspectRatio,
                        QtCore.Qt.SmoothTransformation,
                    )
                self._market_status_icon_cache[icon_cache_key] = market_icon

        icon_gap = 6
        icon_draw_width = (market_icon.width() + icon_gap) if market_icon else 0
        sep = "      "
        sep_width = metrics.horizontalAdvance(sep)
        market_total_width = icon_draw_width + market_text_width + status_text_width + sep_width + 20
        ...
        if market_icon:
            icon_y = (self.ticker_height - market_icon.height()) // 2
            market_painter.drawPixmap(x, icon_y, market_icon)
            x += market_icon.width() + icon_gap
        ...
        market_label_rect = QtCore.QRect(market_label_x, 0, market_text_width, self.ticker_height)
        status_rect = QtCore.QRect(status_x, 0, status_text_width, self.ticker_height)
        market_area_template = [
            ('market_label', 'MARKET', market_label_rect),
            ('market_status', status_text.upper(), status_rect)
        ]

        show_market_status = settings.get("show_market_status", True)
        market_inserted = False

        def append_market_status_item():
            ...
            self.ticker_area_templates.append(market_area_template)

        for tkr in self.stocks:
            # Insert market status after all crypto ($) tickers, before non-crypto tickers
            if show_market_status and not market_inserted and not is_crypto_symbol(tkr):
                append_market_status_item()
                market_inserted = True
            display_name = ticker_display_name(tkr)
            
            price, prev = self.prices.get(tkr, (None, None))
            icon = get_ticker_icon(tkr, icon_size)
            tkr_width = metrics.horizontalAdvance(display_name + " ")
            sparkline_width = max(50, int(self.ticker_height * 0.85)) if self.show_sparklines else 0
            sparkline_gap = 6 if self.show_sparklines else 0
            ...
            if change_text or pct_text:
                ...
                if indicator_style_for_width == "thin_arrows":
                    ...
                    leg_w = max(2, int((indicator_size_for_width / 2) * 0.9))
                    triangle_width = 2 + 2 * leg_w + 6
            else:
                triangle_width = 0
            sep = "      "
            sep_width = metrics.horizontalAdvance(sep)
            total_width = icon_size + 8 + tkr_width + sparkline_gap + sparkline_width + sparkline_gap + price_width + (10 + change_width + triangle_width if change_width else 0) + sep_width + 20
            ...
            if self.show_sparklines and getattr(self, 'sparkline_position', 'left') == 'left':
                ...
                sparkline = self._get_sparkline_pixmap(tkr, sparkline_width, sparkline_height)
                sparkline_y = (self.ticker_height - sparkline.height()) // 2
                painter.drawPixmap(x + sparkline_gap, sparkline_y, sparkline)
                x += sparkline_gap + sparkline_width + sparkline_gap
            ...
            if change_text or pct_text:
                ...
                # Right-align both rows within change_width so their right edges are flush
                change_indent = change_width - small_metrics.horizontalAdvance(change_text)
                pct_indent = change_width - small_metrics.horizontalAdvance(pct_text)
                if glow_color:
                    painter.setPen(glow_color)
                    if USE_OPT:
                        for dx, dy in opt.get_glow_offsets():
                            painter.drawText(x + 10 + change_indent + dx, stacked_top + dy, change_text)
                            painter.drawText(x + 10 + pct_indent + dx, stacked_top + small_metrics.height() + 2 + dy, pct_text)
                    else:
                        for dx in [-2, -1, 0, 1, 2]:
                            for dy in [-2, -1, 0, 1, 2]:
                                if dx != 0 or dy != 0:
                    ...
                    painter.setPen(color)
                    painter.drawText(x + 10 + change_indent, stacked_top, change_text)
                    painter.drawText(x + 10 + pct_indent, stacked_top + small_metrics.height() + 2, pct_text)
                else:
                    # No 5% glow, use global glow if enabled
                    self.draw_text_with_global_glow(painter, x + 10 + change_indent, stacked_top, change_text, color, settings=settings)
                    self.draw_text_with_global_glow(painter, x + 10 + pct_indent, stacked_top + small_metrics.height() + 2, pct_text, color, settings=settings)
                ...
                # Position indicator 2px past the right edge of the stacked block (consistent for all stocks)
                # Triangle is drawn CENTERED at indicator_x; its left edge extends leg_component
                # pixels left of center. Offset by leg_component so left edge = text_right + 2px.
                indicator_size = int(self.ticker_height * 0.42)  # ~42% of ticker height
                leg_component = max(2, int((indicator_size / 2) * 0.9))
                indicator_x = x + 10 + change_width + 2 + leg_component
        ...
        # If market status not yet inserted (all crypto tickers, or empty list), append now
        if show_market_status and not market_inserted:
            append_market_status_item()
    ...
    def build_ticker_pixmaps_for_symbols(self, symbols):
        ...
        for symbol in symbols:
            ...
            if change_text or pct_text:
                ...
                indicator_style = settings.get("price_indicator_style", "triangles")
                # Triangle is drawn CENTERED at indicator_x; left edge = indicator_x - leg_component.
                # Offset by leg_component so left edge lands 2px past text right edge.
            ...
            # Compute the correct index in ticker_pixmaps for this symbol.
            # market_status is inserted before the first non-crypto ticker (or at the
            # end if all tickers are crypto).  Crypto symbols come before the insertion
            # point so they keep their natural 0-based index; non-crypto symbols are
            # shifted right by 1 to account for the market_status slot.
            show_market_status_flag = settings.get("show_market_status", True)
            if show_market_status_flag and not is_crypto_symbol(symbol):
                target_index = idx + 1
            else:
                target_index = idx
            # Ensure lists are long enough
            if target_index < len(self.ticker_pixmaps):
                    # Compensate scroll offset for any width change so segments after this
                    # one don't visually jump left/right when a price gains/loses a digit.
                    # delta > 0 → segment got wider → pull offset left by same amount so
                    # the cycle-start moves left, keeping all higher-index segments in place.
                    old_width = self.ticker_pixmap_widths[target_index]
                    width_delta = total_width - old_width
                    if width_delta != 0 and hasattr(self, 'offset'):
                        self.offset -= width_delta
    ...
    def _request_sparkline_async(self, symbol, width, height):
        ...
        def worker():
            try:
                ...
                encoded_symbol = url_quote(yahoo_symbol, safe='')
    ...
    def apply_glass_glare_effect(self, painter, width, height, settings):
        ...
        The glass glare layer is entirely static — rendered once into a cached
        QPixmap (using Plus composition so overlapping layers stack additively,
        matching the original direct-Plus-on-widget behaviour pixel-for-pixel)
        and blitted each frame with a single drawPixmap call.
        """
        if not settings.get("led_glass_glare", True):
            return

        # Rebuild cache only when window dimensions change (essentially never while docked)
        cache_key = (width, height)
        if not hasattr(self, '_glass_glare_cache') or self._glass_glare_cache_key != cache_key:
            cache = QtGui.QPixmap(width, height)
            cache.fill(QtCore.Qt.transparent)
            cp = QtGui.QPainter(cache)
            cp.setRenderHint(QtGui.QPainter.Antialiasing, False)
            cp.setPen(QtCore.Qt.NoPen)
            # Plus on a transparent surface: each layer adds its premultiplied value,
            # exactly mirroring the sequential Plus draws on the widget painter.
            cp.setCompositionMode(QtGui.QPainter.CompositionMode_Plus)
            ...
            if USE_OPT:
                ...
                for i in range(gradient_stops.shape[0] - 1):
                ...
                cp.setBrush(QtGui.QBrush(glare_gradient_1))
                cp.drawRect(0, 0, width, int(height * 0.33))
                ...
                cp.setBrush(QtGui.QBrush(glare_gradient_2))
                cp.drawPolygon(QtGui.QPolygon([
                ...
                cp.setBrush(QtGui.QBrush(corner_gradient))
                cp.drawRect(0, 0, int(width * 0.3), int(height * 0.33))
                ...
                cp.setBrush(QtGui.QBrush(corner_gradient_2))
                cp.drawRect(int(width * 0.7), int(height * 0.6), int(width * 0.3), int(height * 0.4))
            else:
                glare_gradient_1 = QtGui.QLinearGradient(0, 0, 0, height * 0.33)
            ...
            # Glass texture lines — also Plus so they add to the accumulated layers
            glass_texture_color = QtGui.QColor(255, 255, 255, 5)
            for y in range(0, int(height * 0.33), 15):
                cp.fillRect(0, y, width, 1, glass_texture_color)
            glass_texture_color_light = QtGui.QColor(255, 255, 255, 2)
            for y in range(int(height * 0.33), height, 25):
                cp.fillRect(0, y, width, 1, glass_texture_color_light)

            cp.end()
            self._glass_glare_cache = cache
            self._glass_glare_cache_key = cache_key
            print(f"[PERF] Glass glare cache built ({width}x{height})")

        # Single blit per frame — replaces ~20 painter operations with one drawPixmap call
        painter.setCompositionMode(QtGui.QPainter.CompositionMode_Plus)
        painter.drawPixmap(0, 0, self._glass_glare_cache)
        painter.setCompositionMode(QtGui.QPainter.CompositionMode_SourceOver)
    ...
    def paint_ticker(self, widget):
        ...
        # Phase 2: Frame timing and scroll update moved to TOP of paint_ticker() — before
        # QPainter is created.  Every frame now draws at the position stepped to from THIS
        # VSync timestamp rather than last frame’s, eliminating the 1-frame positional lag.
        # Pop the OLDEST queued VBlank timestamp (FIFO).  In dual-ticker mode the shared
        # vsync thread can emit VBlank N+1 before widget2's paintEvent runs; without the
        # queue, widget2 would read T_N+1 and compute delta = 2 frames → double-jump scroll.
        _vq = getattr(self.gl_widget, '_vsync_queue', None)
        _vsync_time = _vq.popleft() if _vq else None

        if _vsync_time is not None:
            # VSync-triggered paint: advance timing from the VBlank timestamp.
            current_time = _vsync_time
            delta_time = min(current_time - self.last_frame_time, 0.034)
            self.last_frame_time = current_time

            # Jitter measurement: record every VSYNC frame's interval in ms.
            # Judder = variance in frame spacing; std-dev 0 ms = perfectly smooth.
            if not hasattr(self, '_jitter_samples'):
                self._jitter_samples = []
            self._jitter_samples.append(delta_time * 1000.0)

            # FPS counter for diagnostic purposes
            self._fps_counter += 1
        else:
            # Non-vsync repaint (price update, pixmap rebuild, resize, etc.).
            # Redraw the ticker at its CURRENT position — do NOT advance scroll or
            # corrupt last_frame_time; that is reserved for vsync-locked frames only.
            delta_time = 0.0
            current_time = self._time_module.perf_counter()

        # FPS + jitter report — once per second (only meaningful on vsync frames)
        if _vsync_time is not None and current_time - self._fps_last_calc >= 1.0:
            if self._fps_last_calc > 0 and self._fps_counter > 0:
                ...
                # Jitter stats from the samples collected this second.
                # stddev: ideal = 0 ms; <0.5 ms imperceptible; 1-2 ms slight; >3 ms obvious judder.
                # range: worst-case spread between fastest and slowest frame interval.
                _s = self._jitter_samples
                if len(_s) > 1:
                    _mean = sum(_s) / len(_s)
                    _stddev = (sum((_x - _mean) ** 2 for _x in _s) / len(_s)) ** 0.5
                    _range = max(_s) - min(_s)
                    self._current_jitter_stddev = _stddev
                    self._current_jitter_range = _range
                    print(
                        f"[JUDDER] StdDev={_stddev:.2f}ms  Range={_range:.2f}ms"
                        f"  Mean={_mean:.2f}ms  Samples={len(_s)}"
                    )
        ...
        # Scroll update — before drawing so the position rendered this frame is the one
        # advanced from THIS VSync, not the previous frame’s.  Guarded against loading
        # state where get_cycle_width() returns 0 and would corrupt the wraparound math.
        ...
        if USE_OPT:
            # Numba path — Python list is not built (was previously built then discarded)
            ...
            # Pure-Python fallback (only when Numba is unavailable)
            cycle_positions = []
            x = self.offset
            est_cycles = (width // min(base_cycle_width, donate_cycle_width)) + 6
            for i in range(est_cycles):
            ...
            for area_type, tkr, rect in self.ticker_click_areas:
                if rect.contains(pos):
                    if area_type == 'donate':
                        ...
                        # Strip special characters like ^ and $ from ticker symbol for URL
...
class ManageStocksDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        layout.setStretchFactor(self.list_widget, 1)
        ...
        # Market status display toggle
        self.show_market_status_checkbox = QtWidgets.QCheckBox("Show Market Open/Closed Status")
        self.show_market_status_checkbox.setChecked(get_settings().get("show_market_status", True))
        self.show_market_status_checkbox.setStyleSheet("""
            QCheckBox {
                color: #ffffff;
                font-size: 11px;
                spacing: 8px;
        ...
        """)
        layout.addWidget(self.show_market_status_checkbox)
        ...
        # Crypto section
        crypto_label = QtWidgets.QLabel("🪙 Top 20 Crypto Coins (via Yahoo Finance)")
        crypto_label.setStyleSheet("font-size: 12px; font-weight: 600; color: #00b3ff; margin-top: 6px; margin-bottom: 4px;")
        layout.addWidget(crypto_label)

        crypto_layout = QtWidgets.QGridLayout()
        crypto_layout.setHorizontalSpacing(16)
        crypto_layout.setVerticalSpacing(6)
        ...
        for index, (symbol, display_name) in enumerate(TOP_CRYPTO_COINS):
            checkbox = QtWidgets.QCheckBox(f"{display_name} ({symbol})")
            checkbox.setStyleSheet(checkbox_style)
            self.crypto_checkboxes[symbol] = checkbox
            row = index // 2
            col = index % 2
            crypto_layout.addWidget(checkbox, row, col)

        crypto_layout.setColumnStretch(0, 1)
        crypto_layout.setColumnStretch(1, 1)
        ...
        for symbol, checkbox in self.crypto_checkboxes.items():
            checkbox.setChecked(symbol in stock_symbols)
    ...
    def sort_and_refresh(self):
        self.stocks.sort(key=lambda s: stock_sort_key(s[0]))
    ...
    def add_stock(self):
        ...
        if tkr.endswith('-USD') and not tkr.startswith(('^', '$')):
            tkr = f"${tkr}"
    ...
    def check_websocket_cost_warning(self):
        ...
        num_stocks = len(self.stocks)
        finnhub_stocks = len([s for s in self.stocks if not is_yahoo_symbol(s[0])])
    ...
    def save_and_close(self):
        # Save show_market_status setting
        settings = get_settings()
        settings["show_market_status"] = self.show_market_status_checkbox.isChecked()
        save_settings(settings)
        ...
        # Handle top crypto checkboxes
        crypto_map = {symbol: checkbox.isChecked() for symbol, checkbox in self.crypto_checkboxes.items()}
        yahoo_selection_map = {**indices_map, **crypto_map}
        
        # Remove unchecked predefined Yahoo symbols
        self.stocks = [s for s in self.stocks if s[0] not in yahoo_selection_map or yahoo_selection_map[s[0]]]
        
        # Add checked Yahoo symbols that aren't already in the list
        stock_symbols = [s[0] for s in self.stocks]
        for symbol, checked in yahoo_selection_map.items():
...
def parse_args():
    ...
    parser.add_argument("--minimized", action="store_true", help=argparse.SUPPRESS)  # Used by Windows Startup registry entry
...
Examples:
  ...
  TCKR.exe -ht 60 -u 300"""
    ...
    splash = None
    if not getattr(args, 'no_splash', False) and not getattr(args, 'minimized', False):
        splash = SplashScreen()
        splash.show()
